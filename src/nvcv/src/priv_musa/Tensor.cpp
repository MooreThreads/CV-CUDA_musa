                                                ...
                                                NVCVTensorLayout layout, int32_t userBaseAlign, int32_t userRowAlign)
{
    ...
    int dev;
    NVCV_CHECK_THROW(musaGetDevice(&dev));
    ...
    int rowAlign;
    {
        if (userRowAlign == 0)
        {
            // CUDA reports 32-byte texture-pitch alignment on the verifier target used by
            // CV-CUDA's tensor ABI tests. MUSA can report a larger texture pitch alignment,
            // but these strided tensors are plain device-memory allocations exported through
            // the CUDA-compatible tensor data contract, not texture-bound allocations. Keep
            // default row alignment at the CUDA-compatible floor while still honoring dtype
            // element alignment below.
            NVCV_CHECK_THROW(musaDeviceGetAttribute(&rowAlign, musaDevAttrTexturePitchAlignment, dev));
            rowAlign = std::min(rowAlign, 32);
    ...
    // Calculate base address alignment
    {
        if (userBaseAlign == 0)
        {
            ...
            NVCV_CHECK_THROW(musaDeviceGetAttribute(&addrAlign, musaDevAttrTextureAlignment, dev));
