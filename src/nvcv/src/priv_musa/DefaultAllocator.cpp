...
{
    void *ptr = nullptr;
    NVCV_CHECK_THROW(::musaHostAlloc(&ptr, size, musaHostAllocWriteCombined | musaHostAllocMapped));
    // TODO: can we do better than this?
    if (reinterpret_cast<uintptr_t>(ptr) % align != 0)
    {
        NVCV_CHECK_LOG(::musaFreeHost(ptr));
...
{
    void *ptr = nullptr;
    NVCV_CHECK_THROW(::musaMalloc(&ptr, size));

    // TODO: can we do better than this?
    if (reinterpret_cast<uintptr_t>(ptr) % align != 0)
    {
        NVCV_CHECK_LOG(::musaFree(ptr));
        throw Exception(NVCV_ERROR_INTERNAL, "Can't allocate %ld bytes of CUDA memory with alignment at %d bytes", size,
                        align);
    }
    return ptr;
