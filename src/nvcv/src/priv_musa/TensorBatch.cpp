...
void TensorBatch::exportData(MUstream stream, NVCVTensorBatchData &data)
{
    if (m_dirtyBegin < m_dirtyEnd)
    {
        ...
        NVCV_CHECK_THROW(musaMemcpyAsync(m_devTensorsBuffer + m_dirtyBegin, m_pinnedTensorsBuffer + m_dirtyBegin,
                                         copySize, musaMemcpyHostToDevice, stream));
