...
static NVCVResourceType ValidateArrayBuffer(const NVCVArrayData &data)
{
...

    {
        ...
        musaError_t err = musaPointerGetAttributes(&attr, reinterpret_cast<void *>(data.buffer.strided.basePtr));
#if MUSART_VERSION >= 11000
        if (err == musaSuccess && attr.type == musaMemoryTypeUnregistered)
#else  // MUSART_VERSION < 11.0
        if (err == musaErrorInvalidValue)
#endif // MUSART_VERSION
        {
            resource = NVCV_RESOURCE_MEM_HOST;
        }
        else if (err == musaSuccess && (attr.type == musaMemoryTypeDevice || attr.type == musaMemoryTypeManaged))
        {
            resource = NVCV_RESOURCE_MEM_CUDA;
        }
        else if (err == musaSuccess && attr.type == musaMemoryTypeHost)
        {
