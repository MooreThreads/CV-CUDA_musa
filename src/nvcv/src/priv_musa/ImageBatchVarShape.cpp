    ...
    , m_cacheMaxSize{Size2D{0,0}}
{
    ...
    try
    {
        ...
        NVCV_CHECK_THROW(musaEventCreateWithFlags(&m_evPostFence, musaEventDisableTiming));
    }
    catch (...)
    {
        if (m_evPostFence)
        {
            NVCV_CHECK_LOG(musaEventDestroy(m_evPostFence));
        }
...
{
    NVCV_CHECK_LOG(musaEventSynchronize(m_evPostFence));
    clear();
...
void ImageBatchVarShape::exportData(MUstream stream, NVCVImageBatchData &data) const
{
    ...
    {
        NVCV_CHECK_THROW(musaStreamWaitEvent(stream, m_evPostFence));

        NVCV_CHECK_THROW(musaMemcpyAsync(
            m_devImagesBuffer + m_dirtyStartingFromIndex, m_hostImagesBuffer + m_dirtyStartingFromIndex,
            (m_numImages - m_dirtyStartingFromIndex) * sizeof(*m_devImagesBuffer), musaMemcpyHostToDevice, stream));

        NVCV_CHECK_THROW(musaMemcpyAsync(
            m_devFormatsBuffer + m_dirtyStartingFromIndex, m_hostFormatsBuffer + m_dirtyStartingFromIndex,
            (m_numImages - m_dirtyStartingFromIndex) * sizeof(*m_devFormatsBuffer), musaMemcpyHostToDevice, stream));

        // Signal that we finished reading from m_hostBuffer
        NVCV_CHECK_THROW(musaEventRecord(m_evPostFence, stream));
...
void ImageBatchVarShape::pushImages(const NVCVImageHandle *images, int32_t numImages)
{
    ...
    NVCV_CHECK_THROW(musaEventSynchronize(m_evPostFence));
