 ...
 */
template<class T, class = nvcv::musa::Require<nvcv::musa::IsCompound<T>>>
__host__ std::ostream &operator<<(std::ostream &out, const T &v)
{
    using BT      = nvcv::musa::BaseType<T>;
    using OutType = std::conditional_t<sizeof(BT) == 1, int, BT>;

    out << nvcv::musa::GetTypeName<T>() << "(";

    out << static_cast<OutType>(nvcv::musa::GetElement<0>(v));
    if constexpr (nvcv::musa::NumComponents<T> >= 2)
        out << ", " << static_cast<OutType>(nvcv::musa::GetElement<1>(v));
    if constexpr (nvcv::musa::NumComponents<T> >= 3)
        out << ", " << static_cast<OutType>(nvcv::musa::GetElement<2>(v));
    if constexpr (nvcv::musa::NumComponents<T> == 4)
        out << ", " << static_cast<OutType>(nvcv::musa::GetElement<3>(v));
