...
void Stack::operator()(musaStream_t stream, const nvcv::TensorBatch &in, const nvcv::Tensor &out) const
{
...
int Stack::copyTensorToNTensor(const nvcv::TensorDataStridedCuda &outData, const nvcv::TensorDataStridedCuda &inData,
                               uint32_t outIndex, musaStream_t stream) const
{
    ...
    for (uint32_t i = 0; i < in->numSamples(); ++i)
    {
        ...
        {
            NVCV_CHECK_LOG(musaMemcpy2DAsync(
                out->planeData(p, outSampData), out->rowStride(), in->planeData(p, inSampData), in->rowStride(),
                in->numCols() * in->colStride(), in->numRows(), musaMemcpyDeviceToDevice, stream));
        }
        outIndex++;
    }
    return outIndex;
