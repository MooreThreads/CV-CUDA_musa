...
#include <cvcuda/cuda_tools_musa/Printer.hpp>
...
template<typename T>
using DPT = std::conditional_t<musa::NumElements<T> == 3, uint3, uint4>;
...
#pragma unroll
        for (int x = 0; x < NIX<T>; ++x)
        {
            ...
            iSrcCoord.x = musa::min(iSrcCoord.x, srcSizeX - 1);
...
__global__ void NearestResize(SrcWrapper src, DstWrapper dst, int2 srcSize, int2 dstSize, float2 scaleRatio)
{
    ...
    if (dstCoord.y < dstSize.y)
    {
        ...
        float2 srcCoord = (musa::DropCast<2>(dstCoord) + 0.5f) * scaleRatio;
        int3   iSrcCoord{0, (int)floor(srcCoord.y), dstCoord.z};

        iSrcCoord.y = musa::min(iSrcCoord.y, srcSize.y - 1);
...
#pragma unroll
        for (int x = 0; x < NIX<T>; ++x)
        {
            ...
            iSrcCoord.x = musa::max(0, musa::min(iSrcCoord.x, srcSizeX - 2));
            ...
            if constexpr (INTERSECT)
            {
                ...
                dstPack[x]
                    = musa::SaturateCast<T>(srcPack[0] * ((1.f - w.x) * (1.f - w.y)) + srcPack[1] * (w.x * (1.f - w.y))
        ...
        for (int x = 0; x < NIX<T>; ++x)
        {
            if (dstCoordX + x < dstSizeX)
            {
                ...
                if constexpr (INTERSECT)
                {
                    ...
                    dstRow[dstCoordX + x] = musa::SaturateCast<T>(
...
__global__ void LinearResize(SrcWrapper src, DstWrapper dst, int2 srcSize, int2 dstSize, float2 scaleRatio)
{
    ...
    if (dstCoord.y < dstSize.y)
    {
        ...
        float2 srcCoord = (musa::DropCast<2>(dstCoord) + .5f) * scaleRatio - .5f;
        int3   iSrcCoord{0, (int)floor(srcCoord.y), dstCoord.z};
        ...
        iSrcCoord.y = musa::max(0, musa::min(iSrcCoord.y, srcSize.y - 2));
...
{
    using T  = typename DstWrapper::ValueType;
    using FT = nvcv::musa::ConvertBaseTypeTo<float, T>;
    ...
    if (dstCoord.y < dstSize.y && dstCoord.x < dstSize.x)
    {
        ...
        iSrcCoord.y = musa::max(1, musa::min(iSrcCoord.y, srcSize.y - 3));
        iSrcCoord.x = musa::max(1, musa::min(iSrcCoord.x, srcSize.x - 3));
...
#pragma unroll
        ...
        dst[dstCoord] = musa::SaturateCast<T>(musa::abs(sum));
    }
}
...
__global__ void AreaResize(SrcWrapper src, DstWrapper dst, int2 dstSize)
{
    ...
    dst[coord] = src[musa::StaticCast<float>(coord)];
}

// Host run resize functions ---------------------------------------------------

template<typename T>
void RunResizeInterp(musaStream_t stream, const nvcv::TensorDataStridedCuda &srcData,
                     ...
                     const NVCVInterpolationType interpolation)
{
    ...
    auto srcTW = musa::CreateTensorWrapNHW<const T, int32_t>(srcData);
    auto dstTW = musa::CreateTensorWrapNHW<T, int32_t>(dstData);
    auto srcIW = musa::CreateInterpolationWrapNHW<const T, NVCV_BORDER_CONSTANT, NVCV_INTERP_AREA, int32_t>(
...
inline void RunResizeInterpType(musaStream_t stream, const nvcv::TensorDataStridedCuda &srcData,
...
#define CVCUDA_RUN_RESIZE(BT, DT, T)                                             \
    ((srcData.dtype() == nvcv::TYPE_##BT && numChannels == musa::NumElements<T>) \
...
void Resize::RunResize(musaStream_t stream, const nvcv::TensorDataStridedCuda &srcData,
                       const nvcv::TensorDataStridedCuda &dstData, const NVCVInterpolationType interpolation) const
{
    ...
    constexpr int32_t kIntMax = musa::TypeTraits<int32_t>::max;
