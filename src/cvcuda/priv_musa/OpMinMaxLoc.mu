...
#include <cvcuda/cuda_tools_musa/Atomics.hpp>
...
struct OpMin
{
    ...
    using BaseOutType = musa::BaseType<OutType>;

    static constexpr OutType init = {musa::TypeTraits<OutType>::max};
    ...
    {
        a = musa::min(a, musa::StaticCast<BaseOutType>(b));
    }

    template<class OutWrapper>
    __device__ inline static void opAtomic(OutWrapper out, int z, OutType b)
    {
        musa::AtomicMin(get<0>(out)[z].x, b.x);
    }
};
...
struct OpMax
{
    ...
    static constexpr OutType init = {musa::Lowest<OutType>};
    ...
    __device__ inline static void op(OutType &a, U b)
    {
        a = musa::max(a, musa::StaticCast<BaseOutType>(b));
    }

    template<class OutWrapper>
    __device__ inline static void opAtomic(OutWrapper out, int z, OutType b)
    {
        musa::AtomicMax(get<0>(out)[z].x, b.x);
    }
};
...
struct OpMinMax
{
    using BaseOutType = musa::BaseType<OutputType<T>>;
    using OutType     = musa::MakeType<BaseOutType, 2>;

    static constexpr OutType init = {musa::TypeTraits<OutType>::max, musa::Lowest<OutType>};
    ...
    __device__ inline static void op(OutType &a, U b)
    {
        if constexpr (musa::NumElements<U> == 2)
        {
            a.x = musa::min(a.x, static_cast<BaseOutType>(b.x));
            a.y = musa::max(a.y, static_cast<BaseOutType>(b.y));
        }
        else
        {
            static_assert(musa::NumElements<U> == 1 && musa::NumComponents<U> == 1);

            a.x = musa::min(a.x, static_cast<BaseOutType>(b.x));
            a.y = musa::max(a.y, static_cast<BaseOutType>(b.x));
        }
    }
    ...
    {
        musa::AtomicMin(get<0>(out)[z].x, b.x);
        musa::AtomicMax(get<1>(out)[z].x, b.y);
    }
};
...
struct OpCollect1
{
    ...
    {
        return static_cast<musa::BaseType<OutputType<T>>>(data.x) == get<0>(out)[z].x;
    }
...
struct OpCollect2
{
    ...
    {
        return ((static_cast<musa::BaseType<OutputType<T>>>(data.x) == get<0>(out)[z].x)
                | ((static_cast<musa::BaseType<OutputType<T>>>(data.x) == get<1>(out)[z].x) << 16));
    }
...
__global__ __launch_bounds__(BW *BH) void FindMinMax(InWrapper in, int2 size, OutWrapper out)
{
    ...
    if constexpr (!std::is_same_v<InWrapper, musa::Tensor3DWrap<T>>)
    {
...
inline void RunMinMaxLocForType(musaStream_t stream, const DataStridedCuda &inData, OptionalTensorDataRef minValData,
                                ...
                                OptionalTensorDataRef numMaxData)
{
    ...
    using InputWrapper = std::conditional_t<IsTensor, musa::Tensor3DWrap<T>, musa::ImageBatchVarShapeWrap<T>>;
    ...
    if constexpr (IsTensor)
    {
        ...
        inWrap     = musa::CreateTensorWrapNHW<T>(inData);
        inSize     = musa::StaticCast<int>(long2{inAccess->numCols(), inAccess->numRows()});
    ...
    {
        musa::Tensor1DWrap<OutputType<T>, int32_t> minWrap(minValData->get().basePtr());
        musa::Tensor1DWrap<OutputType<T>, int32_t> maxWrap(maxValData->get().basePtr());

        auto outWrap = OutputWrapper(minWrap, maxWrap);

        musa::Tensor2DWrap<int2, int32_t> minLocWrap(minLocData->get().basePtr(), (int)minLocData->get().stride(0));
        musa::Tensor2DWrap<int2, int32_t> maxLocWrap(maxLocData->get().basePtr(), (int)maxLocData->get().stride(0));
        musa::Tensor1DWrap<int1, int32_t> numMinWrap(numMinData->get().basePtr());
        musa::Tensor1DWrap<int1, int32_t> numMaxWrap(numMaxData->get().basePtr());
        ...
        NVCV_CHECK_LOG(musaMemsetAsync(numMinData->get().basePtr(), 0, sizeof(int) * numSamples, stream));
        NVCV_CHECK_LOG(musaMemsetAsync(numMaxData->get().basePtr(), 0, sizeof(int) * numSamples, stream));
...
inline void RunMinMaxLocDataOut(musaStream_t stream, const DataStridedCuda &inData, nvcv::DataType inDataType,
...
template<class DataStridedCuda>
inline void RunMinMaxLocDataIn(musaStream_t stream, const DataStridedCuda &inData, nvcv::DataType inDataType,
                               ...
                               const nvcv::Tensor &maxLoc, const nvcv::Tensor &numMax)
{
    ...
    if (minVal)
    {
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output minVal must be musa-accessible, pitch-linear tensor");
        }
        minLocData = minLoc.exportData<nvcv::TensorDataStridedCuda>();
        if (!minLocData)
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output minLoc must be musa-accessible, pitch-linear tensor");
        }
        numMinData = numMin.exportData<nvcv::TensorDataStridedCuda>();
        if (!numMinData)
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output numMin must be musa-accessible, pitch-linear tensor");
        }
    ...
    if (maxVal)
    {
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output maxVal must be musa-accessible, pitch-linear tensor");
        }
        maxLocData = maxLoc.exportData<nvcv::TensorDataStridedCuda>();
        if (!maxLocData)
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output maxLoc must be musa-accessible, pitch-linear tensor");
        }
        numMaxData = numMax.exportData<nvcv::TensorDataStridedCuda>();
        if (!numMaxData)
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output numMax must be musa-accessible, pitch-linear tensor");
        }
...
void MinMaxLoc::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &minVal,
                           const nvcv::Tensor &minLoc, const nvcv::Tensor &numMin, const nvcv::Tensor &maxVal,
...
void MinMaxLoc::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in, const nvcv::Tensor &minVal,
                           const nvcv::Tensor &minLoc, const nvcv::Tensor &numMin, const nvcv::Tensor &maxVal,
                           const nvcv::Tensor &maxLoc, const nvcv::Tensor &numMax) const
{
