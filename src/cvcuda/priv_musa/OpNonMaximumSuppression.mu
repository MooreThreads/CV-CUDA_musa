...
{
    int   xInterLeft   = musa::max(box1.x, box2.x);
    int   yInterTop    = musa::max(box1.y, box2.y);
    int   xInterRight  = musa::min(box1.x + box1.z, box2.x + box2.z);
    int   yInterBottom = musa::min(box1.y + box1.w, box2.y + box2.w);
...
__global__ void NonMaximumSuppression(musa::Tensor2DWrap<const T, int32_t>     inBBoxes,
                                      musa::Tensor2DWrap<U, int32_t>           outMask,
                                      musa::Tensor2DWrap<const float, int32_t> inScores, int numBBoxes,
...
inline __host__ void RunNonMaximumSuppresion(const nvcv::TensorDataStridedCuda &in,
                                             ...
                                             musaStream_t stream)
{
    musa::Tensor2DWrap<const short4, int32_t> inWrap(in);
    musa::Tensor2DWrap<uint8_t, int32_t>      outWrap(out);
    musa::Tensor2DWrap<const float, int32_t>  scoresWrap(scores);
...
void NonMaximumSuppression::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
                                       const nvcv::Tensor &scores, float scoreThreshold, float iouThreshold) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Scores must be musa-accessible, pitch-linear tensor");
    }
