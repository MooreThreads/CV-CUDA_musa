...
#include <cvcuda/cuda_tools_musa/InterpolationWrap.hpp>
...
template<typename T>
using TensorWrapLNHW = musa::TensorWrap32<T, -1, -1, -1, sizeof(T)>;

// Border wrap for LNHWC tensors using the corresponding tensor wrap
template<typename T>
using BorderWrapLNHW = musa::BorderWrap<TensorWrapLNHW<T>, kBorderGauss, false, false, true, true>;

// Tensor wrap for descriptor has 2 compile-time strides: per 128B descriptor and per 1B within descriptor
using TensorWrapForDescriptor = musa::TensorWrap32<uint8_t, -1, kDescSize * sizeof(uint8_t), sizeof(uint8_t)>;
...
{
    return musa::min(musa::round<int>(sigma * 8 + 1) | 1, kMaxKernelSize);
}
...
{
    int3 srcCoord = {static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x),
...
__global__ void DoComputePyramids(BorderWrapLNHW<float> prevGauss, TensorWrapLNHW<float> currGauss,
                                  ...
                                  musa::math::Vector<float, kMaxKernelSize> gaussSepKernel)
{
    ...
    musa::TensorWrap32<float, SW * sizeof(float), sizeof(float)> gaussIn(&gaussInData[0]);
    musa::TensorWrap32<float, BW * sizeof(float), sizeof(float)> gaussOut(&gaussOutData[0]);
    ...
    int4 gc{0, 0, (int)blockIdx.z, musa::max(0, currLayer - 1)}; // current (global) coordinates for GMEM data
...
__global__ void DoComputeDescriptors(TensorWrapForDescriptor             featDescriptors,
                                     musa::Tensor2DWrap<float4, int32_t> featCoords,
                                     musa::Tensor2DWrap<float3, int32_t> featMetadata,
                                     musa::Tensor1DWrap<int, int32_t>    numFeatures,
                                     ...
                                     float unscaleOctave)
{
    ...
    // Using TensorWrap with compile-time strides for easy multi-dimensional access of Gaussian data in SMEM
    musa::TensorWrap32<float, BW * sizeof(float), sizeof(float)> gaussIn(&gaussInData[0]);
    ...
    int2 featCoord{musa::round<int>(pFeatCoords[0] * unscaleOctave), musa::round<int>(pFeatCoords[1] * unscaleOctave)};
    int  featLayer{musa::round<int>(pFeatCoords[3])};
    ...
    int    radius        = musa::round<int>(histWidth * kDescWidthToRadius);
    ...
    if (musa::abs(featAngle - 360.f) < 1e-5)
    {
        featAngle = 0.f;
    }
    ...
    for (int ti = threadIdx.x; ti < (radius * 2 + 1) * (radius * 2 + 1); ti += blockDim.x)
    {
        ...
        int iHist = musa::round<musa::RoundMode::DOWN, int>(r_rot); // i index in histogram
        int jHist = musa::round<musa::RoundMode::DOWN, int>(c_rot); // j index in histogram
        r_rot -= iHist;
        c_rot -= jHist;
        ...
        int bin = musa::round<musa::RoundMode::DOWN, int>(o_rot); // rotate index for histogram bin
        o_rot -= bin;
        if (bin < 0)
            bin += kDescHistBins;
    ...
    norm = kDescF32toU8Ratio / musa::max(::sqrtf(norm), 1e-5f);
    ...
    for (int i = 0; i < kDescWidth; i++)
    {
        for (int j = 0; j < kDescWidth; j++)
        {
            ...
            for (int bin = 0; bin < kDescHistBins; bin++)
            {
                ...
                *featDescriptors.ptr(sampleIdx, featIdx, descIdx) = musa::SaturateCast<uint8_t>(v * norm);
            }
        }
    }
}
...
__global__ void DoFindExtrema(musa::Tensor2DWrap<float4, int32_t> featCoords,
                              musa::Tensor2DWrap<float3, int32_t> featMetadata, int maxCapacity,
                              musa::Tensor1DWrap<int, int32_t> numFeatures, TensorWrapLNHW<const float> currGauss,
                              ...
                              float initSigma)
{
    constexpr float kImageScale = 1.f / musa::TypeTraits<DT>::max; // source images data type scale
    ...
    musa::math::Vector<float, 3>    dD, sol; // derivative distances and solver solution
    musa::math::Matrix<float, 3, 3> H;       // Hessian matrix
        ...
        for (gc.w = 1; gc.w <= numOctaveLayers; gc.w++)
        {
                  ...
                  cv <= currDoG[adj(gc, -1, +1, +0)] && cv <= currDoG[adj(gc, -1, +1, +1)])))
            {
                ...
                for (int i = 0; i < kMaxInterpolationSteps; i++)
                {
                    ...
                    if (!musa::math::solve_inplace(H, sol))
                    {
                    ...
                    if (musa::abs(sol[2]) < 0.5f && musa::abs(sol[1]) < 0.5f && musa::abs(sol[0]) < 0.5f)
                    {
                        converged = true;
                        break; // less than half a pixel step means converged
                    }

                    if (musa::abs(sol[2]) > musa::TypeTraits<int>::max / 3.f
                        || musa::abs(sol[1]) > musa::TypeTraits<int>::max / 3.f
                        || musa::abs(sol[0]) > musa::TypeTraits<int>::max / 3.f)
                    {
                        break; // too big a step, convergence is not possible
                    }

                    ic.x += musa::round<int>(sol[0]);
                    ic.y += musa::round<int>(sol[1]);
                    ic.w += musa::round<int>(sol[2]);
                ...
                float featScore = musa::abs(cv * kImageScale + musa::math::dot(dD, sol) * .5f);
                ...
                int radius = musa::round<int>(featRadius * kOrientationRadius);
                ...
                for (int i = -radius; i <= radius; i++)
                {
                    ...
                    for (int j = -radius; j <= radius; j++)
                    {
                        ...
                        int bin = musa::round<int>(angle * kAngleToBin); // the angle indexes the histogram bin
                ...
                {
                    histPeak = musa::max(histPeak, hist[i]);
                }
...
void SIFT::FindExtrema(const nvcv::TensorDataStridedCuda &featCoordsData,
                       ...
                       float initSigma, musaStream_t stream) const
{
    ...
    musa::Tensor2DWrap<float4, int32_t> featCoordsWrap(featCoordsData.basePtr(), (int)featCoordsData.stride(0));
    musa::Tensor2DWrap<float3, int32_t> featMetadataWrap(featMetadataData.basePtr(), (int)featMetadataData.stride(0));
    musa::Tensor1DWrap<int, int32_t>    numFeaturesWrap(numFeaturesData.basePtr());
    ...
    NVCV_CHECK_THROW(musaMemsetAsync(numFeaturesData.basePtr(), 0, sizeof(int) * currShape.z, stream));
...
void SIFT::ComputePyramids(const nvcv::TensorDataStridedCuda &inData, int3 currShape, bool expandInput, int numOctaves,
                           int numOctaveLayers, float initSigma, musaStream_t stream) const
{
    ...
    if (expandInput)
    {
        auto srcBaseWrap = musa::CreateInterpolationWrapNHW<const DT, kBorderInterp, kInterpUp, int32_t>(inData);

        UpCopy<<<copyBlocks, copyThreads, 0, stream>>>(dstBaseWrap, srcBaseWrap, currShape); // upscale copy
    }
    else
    {
        auto srcBaseWrap = musa::CreateTensorWrapNHW<const DT, int32_t>(inData);
    ...
    float currSigma  = musa::sqrt(musa::max(initSigma * initSigma - kPrevSigma * kPrevSigma * sigmaScale, kMinSigma));
    float baseSigma  = musa::pow(2.f, 1.f / numOctaveLayers);

    musa::math::Vector<float, kMaxKernelSize> gaussSepKernel; // separable Gaussian filter kernel
        ...
        for (int layer = 0; layer < numOctaveLayers + 3; layer++)
        {
            ...
            else
            {
                if (layer > 0)
                {
                    ...
                    float prevSigma  = musa::pow(baseSigma, (float)(layer - 1)) * initSigma;
...
void SIFT::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &featCoords,
                      ...
                      float edgeThreshold, float initSigma, NVCVSIFTFlagType flags) const
{
    ...
    if (inAccess->sampleStride() * inAccess->numSamples() > musa::TypeTraits<int32_t>::max)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_OVERFLOW, "Input size exceeds %d. Tensor is too large.",
                              musa::TypeTraits<int32_t>::max);
    }
