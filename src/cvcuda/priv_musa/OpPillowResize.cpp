...
{
    m_legacyOp         = std::make_unique<leg::musa_op::PillowResize>();
    m_legacyOpVarShape = std::make_unique<leg::musa_op::PillowResizeVarShape>();
}
                                                             ...
                                                             nvcv::Size2D maxOutSize, NVCVImageFormat fmt)
{
    ...
    leg::musa_op::DataType  dataType = leg::helpers::GetLegacyDataType(bpc[0], dkind);
    leg::musa_op::DataShape maxIn(maxBatchSize, maxChannel, maxInSize.h, maxInSize.w);
    leg::musa_op::DataShape maxOut(maxBatchSize, maxChannel, maxOutSize.h, maxOutSize.w);
...
void PillowResize::operator()(musaStream_t stream, const Workspace &ws, const nvcv::Tensor &in, const nvcv::Tensor &out,
...
void PillowResize::operator()(musaStream_t stream, const Workspace &ws, const nvcv::ImageBatchVarShape &in,
