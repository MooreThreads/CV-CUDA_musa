...
{
    return musa::abs(a.x) + musa::abs(a.y);
}

static __device__ __forceinline__ float norm1(const float3 &a)
{
    return musa::abs(a.x) + musa::abs(a.y) + musa::abs(a.z);
}

static __device__ __forceinline__ float norm1(const float4 &a)
{
    return musa::abs(a.x) + musa::abs(a.y) + musa::abs(a.z) + musa::abs(a.w);
}

template<class SrcWrapper, class DstWrapper>
__global__ void JointBilateralFilterVarShapeKernel(const SrcWrapper src, const SrcWrapper srcColor, DstWrapper dst,
                                                   const musa::Tensor1DWrap<int>   inDiameter,
                                                   const musa::Tensor1DWrap<float> inSigmaColor,
                                                   const musa::Tensor1DWrap<float> inSigmaSpace)
{
    ...
    work_type centerColor0 = musa::StaticCast<float>(srcColor[coord0]);
    work_type centerColor1 = musa::StaticCast<float>(srcColor[coord1]);
    work_type centerColor2 = musa::StaticCast<float>(srcColor[coord2]);
    work_type centerColor3 = musa::StaticCast<float>(srcColor[coord3]);
    ...
    work_type numerator0        = musa::SetAll<work_type>(0);
    work_type numerator1        = musa::SetAll<work_type>(0);
    work_type numerator2        = musa::SetAll<work_type>(0);
    work_type numerator3        = musa::SetAll<work_type>(0);
    ...
    for (int c = colIdx - radius; c < colIdx + radius + 2; c++)
    {
        for (int r = rowIdx - radius; r < rowIdx + radius + 2; r++)
        {
            ...
            work_type curr      = musa::StaticCast<float>(src[srcCoord]);
            work_type currColor = musa::StaticCast<float>(srcColor[srcCoord]);
            ...
            if (squared_dis0 <= squared_radius)
            {
                ...
                float weight        = musa::exp(e_space + e_color);
                denominator0 += weight;
    ...
    {
        *dst.ptr(coord0.z, coord0.y, coord0.x) = nvcv::musa::SaturateCast<T>(numerator0 / denominator0);
    }
    if (colIdx + 1 < columns && rowIdx < rows)
    {
        *dst.ptr(coord1.z, coord1.y, coord1.x) = nvcv::musa::SaturateCast<T>(numerator1 / denominator1);
    }
    if (colIdx < columns && rowIdx + 1 < rows)
    {
        *dst.ptr(coord2.z, coord2.y, coord2.x) = nvcv::musa::SaturateCast<T>(numerator2 / denominator2);
    }
    if (colIdx + 1 < columns && rowIdx + 1 < rows)
    {
        *dst.ptr(coord3.z, coord3.y, coord3.x) = nvcv::musa::SaturateCast<T>(numerator3 / denominator3);
    }
}
...
void JointBilateralFilterVarShapeCaller(const ImageBatchVarShapeDataStridedCuda &inData,
                                        ...
                                        const musa::Tensor1DWrap<int>   &inDiameter,
                                        const musa::Tensor1DWrap<float> &inSigmaColor,
                                        const musa::Tensor1DWrap<float> &inSigmaSpace, musaStream_t stream)
{
    musa::BorderVarShapeWrap<const T, B> src(inData);
    musa::BorderVarShapeWrap<const T, B> srcColor(inColorData);
    musa::ImageBatchVarShapeWrap<T>      dst(outData);
                                              ...
                                              musaStream_t stream)
{
    ...
    musa_op::DataFormat input_format      = GetLegacyDataFormat(inData);
    musa_op::DataFormat inputColor_format = GetLegacyDataFormat(inColorData);
    musa_op::DataFormat output_format     = GetLegacyDataFormat(outData);
    ...
    musa::Tensor1DWrap<int>   inDiameter(diameterData);
    musa::Tensor1DWrap<float> inSigmaColor(sigmaColorData);
    musa::Tensor1DWrap<float> inSigmaSpace(sigmaSpaceData);
    ...
    typedef void (*joint_bilateral_filter_var_shape_t)(
        ...
        const ImageBatchVarShapeDataStridedCuda &outData, int batch, const musa::Tensor1DWrap<int> &inDiameter,
        const musa::Tensor1DWrap<float> &inSigmaColor, const musa::Tensor1DWrap<float> &inSigmaSpace,
        musaStream_t stream);
