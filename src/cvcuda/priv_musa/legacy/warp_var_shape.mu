...
__global__ void inverseMatWarpPerspective(const int numImages, const musa::Tensor2DWrap<float, int32_t> in,
                                          musa::Tensor2DWrap<float, int32_t> out)
{
    ...
    musa::math::Matrix<float, 3, 3> transMatrix;

    transMatrix.load(in.ptr(index));

    musa::math::inv_inplace(transMatrix);

    transMatrix.store(out.ptr(index));
}

__global__ void inverseMatWarpAffine(const int numImages, const musa::Tensor2DWrap<float, int32_t> in,
                                     musa::Tensor2DWrap<float, int32_t> out)
{
    int index = threadIdx.x + blockIdx.x * blockDim.x;
    if (index >= numImages)
...
__global__ void warp(SrcWrapper src, DstWrapper dst, const musa::Tensor2DWrap<float, int32_t> coeffs)
{
...
struct WarpDispatcher
{
    static void call(const ImageBatchVarShapeDataStridedCuda &inData, const ImageBatchVarShapeDataStridedCuda &outData,
                     const musa::Tensor2DWrap<float, int32_t> transform, const float4 &borderValue, musaStream_t stream)
    {
        ...
        auto bVal = musa::StaticCast<musa::BaseType<T>>(musa::DropCast<musa::NumElements<T>>(borderValue));

        musa::InterpolationVarShapeWrap<const T, B, I> src(inData, bVal);
...
void warp_caller(const ImageBatchVarShapeDataStridedCuda &inData, const ImageBatchVarShapeDataStridedCuda &outData,
                 musa::Tensor2DWrap<float, int32_t> transform, const int interpolation, const int borderMode,
                 const float4 &borderValue, musaStream_t stream)
{
    typedef void (*func_t)(
        ...
        musa::Tensor2DWrap<float, int32_t> transform, const float4 &borderValue, musaStream_t stream);
    ...
    , m_maxBatchSize(maxBatchSize)
{
    if (m_maxBatchSize > 0)
    {
        ...
        NVCV_CHECK_LOG(musaMalloc(&m_transformationMatrix, bufferSize));
    }
}
...
WarpAffineVarShape::~WarpAffineVarShape()
{
    ...
    {
        NVCV_CHECK_LOG(musaFree(m_transformationMatrix));
    }
...
ErrorCode WarpAffineVarShape::infer(const ImageBatchVarShapeDataStridedCuda &inData,
                                    ...
                                    const NVCVBorderType borderMode, const float4 borderValue, musaStream_t stream)
{
    if (m_maxBatchSize <= 0)
    {
    ...
    musa::Tensor2DWrap<float, int32_t> transMatrixInput(transMatrix);
    musa::Tensor2DWrap<float, int32_t> transMatrixOutput(m_transformationMatrix, static_cast<int>(sizeof(float) * 9));
    ...
    {
        NVCV_CHECK_LOG(musaMemcpy2DAsync(m_transformationMatrix, sizeof(float) * 9, transMatrixInput.ptr(0, 0),
    ...
    typedef void (*func_t)(const ImageBatchVarShapeDataStridedCuda &inData,
                           const ImageBatchVarShapeDataStridedCuda &outData,
                           const musa::Tensor2DWrap<float, int32_t> transform, const int interpolation,
                           const int borderMode, const float4 &borderValue, musaStream_t stream);
