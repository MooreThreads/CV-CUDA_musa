...
__global__ void composite_kernel(const musa::ImageBatchVarShapeWrap<T> fg, const musa::ImageBatchVarShapeWrap<T> bg,
                                 const musa::ImageBatchVarShapeWrap<U> fgMask, musa::ImageBatchVarShapeWrap<D> dst)
{
    ...
    constexpr int dst_ch = musa::NumElements<D>;
    constexpr int src_ch = musa::NumElements<T>;
    ...
    {
        uint8_t c0               = musa::GetElement(bg_val, i);
        uint8_t c1               = musa::GetElement(fg_val, i);
        musa::GetElement(out, i) = AlphaLerp(c0, c1, mask_val);
    }
    if (src_ch == 3 && dst_ch == 4)
        musa::GetElement(out, 3) = 255;
...
void composite(const nvcv::ImageBatchVarShapeDataStridedCuda &foregroundData,
               ...
               const nvcv::ImageBatchVarShapeDataStridedCuda &outData, musaStream_t stream)
{
    typedef typename musa::MakeType<T, scn> src_type;
    typedef typename musa::MakeType<T, dcn> dst_type;

    musa::ImageBatchVarShapeWrap<src_type> fg_ptr(foregroundData);
    musa::ImageBatchVarShapeWrap<src_type> bg_ptr(backgroundData);
    musa::ImageBatchVarShapeWrap<T>        fgMask_ptr(fgMaskData);
    musa::ImageBatchVarShapeWrap<dst_type> dst_ptr(outData);
...
ErrorCode CompositeVarShape::infer(const ImageBatchVarShapeDataStridedCuda &foreground,
                                   ...
                                   const ImageBatchVarShapeDataStridedCuda &outData, musaStream_t stream)
{
