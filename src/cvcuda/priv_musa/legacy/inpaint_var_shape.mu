...
void sub_helper(Ptr2dNHWC<T> src1, Ptr2dNHWC<T> src2, Ptr2dNHWC<T> dst, nvcv::Size2D maxsize,
                ImageBatchVarShapeWrapNHWC<T> size_helper, musaStream_t stream)
{
    dim3 block(16, 16);
    dim3 grid(divUp(maxsize.w + 2, block.x), divUp(maxsize.h + 2, block.y), dst.batches);
...
void set_value(Ptr2dNHWC<T> src, Ptr2dNHWC<T2> mask, T value, nvcv::Size2D maxsize,
               ImageBatchVarShapeWrapNHWC<T2> size_helper, musaStream_t stream)
{
    dim3 block(16, 16);
...
inline int finish_flag_reduce(Ptr2dNHWC<unsigned char> src_ptr, int *d_out, int *d_out2,
                              ImageBatchVarShapeWrapNHWC<unsigned char> size_helper, musaStream_t stream)
{
...
void inpaint_helper(const nvcv::ImageBatchVarShapeDataStridedCuda &inData,
                    ...
                    int range, bool &init_flag, int channel, int maxBatchSize, musaStream_t stream)
{
    ...
    checkCudaErrors(musaMemsetAsync(band_ptr, KNOWN, sizeof(unsigned char) * batch * erows * ecols * 1, stream));
...
ErrorCode InpaintVarShape::infer(const nvcv::ImageBatchVarShape          &inBatch,
                                 ...
                                 const nvcv::ImageBatchVarShape &outBatch, double inpaintRadius, musaStream_t stream)
{
    ...
    typedef void (*inpaint_t)(
        ...
        bool &init_flag, int channel, int maxBatchSize, musaStream_t stream);
