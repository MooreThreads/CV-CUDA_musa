...
__global__ void filter2D(const SrcWrapper src, DstWrapper dst, musa::ImageBatchVarShapeWrap<float> kernel,
                         musa::Tensor1DWrap<int2, int32_t> kernelAnchor)
{
    using work_type = musa::ConvertBaseTypeTo<float, typename DstWrapper::ValueType>;
    work_type res   = musa::SetAll<work_type>(0);
    ...
    *dst.ptr(batch_idx, y, x) = musa::SaturateCast<typename DstWrapper::ValueType>(res);
}
...
void Filter2DCaller(const ImageBatchVarShapeDataStridedCuda &inData, const ImageBatchVarShapeDataStridedCuda &outData,
                    ...
                    float borderValue, musaStream_t stream)
{
    musa::BorderVarShapeWrap<const D, B> src(inData, musa::SetAll<D>(borderValue));
    musa::ImageBatchVarShapeWrap<D>      dst(outData);
    musa::ImageBatchVarShapeWrap<float>  kernel(kernelData);
    musa::Tensor1DWrap<int2, int32_t>    kernelAnchor(kernelAnchorData);

    using work_type = musa::ConvertBaseTypeTo<float, D>;
...
void Filter2D(const ImageBatchVarShapeDataStridedCuda &inData, const ImageBatchVarShapeDataStridedCuda &outData,
              const ImageBatchVarShapeDataStridedCuda &kernelData, const TensorDataStridedCuda &kernelAnchorData,
              NVCVBorderType borderMode, float borderValue, musaStream_t stream)
{
    typedef void (*func_t)(const ImageBatchVarShapeDataStridedCuda &inData,
                           ...
                           const TensorDataStridedCuda &kernelAnchorData, float borderValue, musaStream_t stream);
...
__device__ musa::math::Vector<float, 9> kLaplacianKernel1{
    {0.0f,  1.0f, 0.0f,
     1.0f, -4.0f, 1.0f,
     0.0f,  1.0f, 0.0f}
};
__device__ musa::math::Vector<float, 9> kLaplacianKernel3{
    {2.0f,  0.0f, 2.0f,
     0.0f, -8.0f, 0.0f,
     2.0f,  0.0f, 2.0f}
};
...
template<class SrcWrapper, class DstWrapper>
__global__ void laplacianFilter2D(const SrcWrapper src, DstWrapper dst, musa::Tensor1DWrap<int, int32_t> ksize,
                                  musa::Tensor1DWrap<float, int32_t> scale)
{
    using work_type = musa::ConvertBaseTypeTo<float, typename DstWrapper::ValueType>;
    work_type res   = musa::SetAll<work_type>(0);
    ...
    musa::math::Vector<float, 9> kernel = ksizeVal == 1 ? kLaplacianKernel1 : kLaplacianKernel3;
...
void LaplacianFilter2DCaller(const ImageBatchVarShapeDataStridedCuda &inData,
                             ...
                             const TensorDataStridedCuda &scale, float borderValue, musaStream_t stream)
{
    musa::BorderVarShapeWrap<const D, B> src(inData, musa::SetAll<D>(borderValue));
    musa::ImageBatchVarShapeWrap<D>      dst(outData);
    musa::Tensor1DWrap<int, int32_t>     kernelApertureSize(ksize);
    musa::Tensor1DWrap<float, int32_t>   kernelScale(scale);
...
ErrorCode LaplacianVarShape::infer(const ImageBatchVarShapeDataStridedCuda &inData,
                                   const ImageBatchVarShapeDataStridedCuda &outData, const TensorDataStridedCuda &ksize,
                                   const TensorDataStridedCuda &scale, NVCVBorderType borderMode, musaStream_t stream)
{
    if (!inData.uniqueFormat())
    {
        LOG_ERROR("Images in the input batch must all have the same format");
...
template<class SrcWrapper, class DstWrapper>
__global__ void gaussianFilter2D(const SrcWrapper src, DstWrapper dst, musa::Tensor3DWrap<float, int32_t> kernel,
                                 musa::Tensor1DWrap<int2, int32_t> kernelSizeArr, Size2D maxKernelSize,
                                 musa::Tensor1DWrap<double2, int32_t> sigmaArr, int dataKernelSize)
{
    using work_type = musa::ConvertBaseTypeTo<float, typename DstWrapper::ValueType>;
    work_type res   = musa::SetAll<work_type>(0);
...
void GaussianFilter2DCaller(const ImageBatchVarShapeDataStridedCuda  &inData,
                            const ImageBatchVarShapeDataStridedCuda  &outData,
                            const musa::Tensor3DWrap<float, int32_t> &kernelTensor,
                            const musa::Tensor1DWrap<int2, int32_t> &kernelSizeTensor, Size2D maxKernelSize,
                            const musa::Tensor1DWrap<double2, int32_t> &sigmaTensor, int dataKernelSize,
                            float borderValue, musaStream_t stream)
{
    musa::BorderVarShapeWrap<const D, B> src(inData, musa::SetAll<D>(borderValue));
    musa::ImageBatchVarShapeWrap<D>      dst(outData);
    ...
    , m_maxBatchSize(maxBatchSize)
{
    if (maxBatchSize > 0)
    {
        NVCV_CHECK_THROW(musaMalloc(&m_kernel, maxKernelSize.w * maxKernelSize.h * maxBatchSize * sizeof(float)));
    }
}

GaussianVarShape::~GaussianVarShape()
{
    NVCV_CHECK_LOG(musaFree(m_kernel));
}
                                  ...
                                  NVCVBorderType borderMode, musaStream_t stream)
{
    ...
    musa::Tensor1DWrap<int2, int32_t>    kernelSizeTensor(kernelSize);
    musa::Tensor1DWrap<double2, int32_t> sigmaTensor(sigma);

    int kernelPitch2 = static_cast<int>(m_maxKernelSize.w * sizeof(float));
    int kernelPitch1 = m_maxKernelSize.h * kernelPitch2;

    musa::Tensor3DWrap<float, int32_t> kernelTensor(m_kernel, kernelPitch1, kernelPitch2);
...
template<class SrcWrapper, class DstWrapper>
__global__ void avgBlurFilter2D(const SrcWrapper src, DstWrapper dst, musa::Tensor3DWrap<float, int32_t> kernel,
...
void AverageBlurFilter2DCaller(const ImageBatchVarShapeDataStridedCuda  &inData,
                               const ImageBatchVarShapeDataStridedCuda  &outData,
                               const musa::Tensor3DWrap<float, int32_t> &kernelTensor,
                               const musa::Tensor1DWrap<int2, int32_t>  &kernelSizeTensor,
                               const musa::Tensor1DWrap<int2, int32_t> &kernelAnchorTensor, float borderValue,
                               musaStream_t stream)
{
    musa::BorderVarShapeWrap<const D, B> src(inData, musa::SetAll<D>(borderValue));
    musa::ImageBatchVarShapeWrap<D>      dst(outData);
...
void AverageBlurFilter2D(const ImageBatchVarShapeDataStridedCuda  &inData,
                         ...
                         const musa::Tensor3DWrap<float, int32_t> &kernelTensor,
                         const musa::Tensor1DWrap<int2, int32_t>  &kernelSizeTensor,
                         const musa::Tensor1DWrap<int2, int32_t> &kernelAnchorTensor, NVCVBorderType borderMode,
                         float borderValue, musaStream_t stream)
{
    typedef void (*func_t)(
        const ImageBatchVarShapeDataStridedCuda &inData, const ImageBatchVarShapeDataStridedCuda &outData,
        const musa::Tensor3DWrap<float, int32_t> &kernelTensor,
        const musa::Tensor1DWrap<int2, int32_t>  &kernelSizeTensor,
        const musa::Tensor1DWrap<int2, int32_t> &kernelAnchorTensor, float borderValue, musaStream_t stream);
                                     ...
                                     NVCVBorderType borderMode, musaStream_t stream)
{
    ...
    musa::Tensor1DWrap<int2, int32_t> kernelSizeTensor(kernelSize);
    musa::Tensor1DWrap<int2, int32_t> kernelAnchorTensor(kernelAnchor);
    ...
    typedef void (*filter2D_t)(const ImageBatchVarShapeDataStridedCuda  &inData,
                               ...
                               const musa::Tensor3DWrap<float, int32_t> &kernelTensor,
                               const musa::Tensor1DWrap<int2,  int32_t> &kernelSizeTensor,
                               const musa::Tensor1DWrap<int2,  int32_t> &kernelAnchorTensor,
