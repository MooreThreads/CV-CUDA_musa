...
template<typename D>
__global__ void erase(nvcv::musa::ImageBatchVarShapeWrapNHWC<D> img, nvcv::musa::Tensor1DWrap<int2> anchorVec,
        ...
        if ((anchor.x + x) < img.width(batchId) && (anchor.y + y) < img.height(batchId))
        {
            if (random)
            {
                ...
                *img.ptr(batchId, anchor.y + y, anchor.x + x, c)
                    = nvcv::musa::SaturateCast<D>(erase_var_shape_hash(hashValue) % 256);
            }
            else
            {
                *img.ptr(batchId, anchor.y + y, anchor.x + x, c) = nvcv::musa::SaturateCast<D>(value);
            }
        }
    }
}
...
void eraseCaller(const nvcv::ImageBatchVarShapeDataStridedCuda &imgs, const nvcv::TensorDataStridedCuda &anchor,
                 ...
                 unsigned int seed, musaStream_t stream)
{
    nvcv::musa::ImageBatchVarShapeWrapNHWC<D> src(imgs, imgs.uniqueFormat().numChannels());
                               ...
                               unsigned int seed, bool inplace, musaStream_t stream)
{
    ...
    typedef void (*erase_t)(const ImageBatchVarShapeDataStridedCuda &imgs, const TensorDataStridedCuda &anchor,
                            ...
                            bool random, unsigned int seed, musaStream_t stream);
