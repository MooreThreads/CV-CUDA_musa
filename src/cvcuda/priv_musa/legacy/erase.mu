...
__global__ void erase(Wrapper img, int imgH, int imgW, nvcv::musa::Tensor1DWrap<int2> anchorVec,
                      nvcv::musa::Tensor1DWrap<int3> erasingVec, nvcv::musa::Tensor1DWrap<float> valuesVec,
                      nvcv::musa::Tensor1DWrap<int> imgIdxVec, int channels, int random, unsigned int seed)
{
        ...
        if (anchor.x + x < imgW && anchor.y + y < imgH)
        {
            if (random)
            {
                ...
                *img.ptr(batchId, anchor.y + y, anchor.x + x, c)
                    = nvcv::musa::SaturateCast<T>(erase_hash(hashValue) % 256);
            }
            else
            {
                *img.ptr(batchId, anchor.y + y, anchor.x + x, c) = nvcv::musa::SaturateCast<T>(value);
            }
        }
    }
}
...
void eraseCaller(const nvcv::TensorDataStridedCuda &imgs, const nvcv::TensorDataStridedCuda &anchor,
                 ...
                 unsigned int seed, int rows, int cols, int channels, musaStream_t stream)
{
    auto wrap = nvcv::musa::CreateTensorWrapNHWC<T>(imgs);

    nvcv::musa::Tensor1DWrap<int2>  anchorVec(anchor);
    nvcv::musa::Tensor1DWrap<int3>  erasingVec(erasing);
    nvcv::musa::Tensor1DWrap<int>   imgIdxVec(imgIdx);
    nvcv::musa::Tensor1DWrap<float> valuesVec(values);
...
{
    musaError_t err = musaMalloc(&d_max_values, sizeof(int3));
    if (err != musaSuccess)
    {
    ...
    err = musaMalloc(&temp_storage, storage_bytes);
    if (err != musaSuccess)
    {
        musaFree(d_max_values);
...
Erase::~Erase()
{
    musaError_t err0 = musaFree(d_max_values);
    musaError_t err1 = musaFree(temp_storage);
                       ...
                       unsigned int seed, bool inplace, musaStream_t stream)
{
    ...
    cub::DeviceReduce::Reduce(temp_storage, storage_bytes, d_erasing, d_max_values, num_erasing_area, maxwh, init,
                              stream);
    checkCudaErrors(musaMemcpyAsync(&h_max_values, d_max_values, sizeof(int3), musaMemcpyDeviceToHost, stream));
    ...
    typedef void (*erase_t)(const TensorDataStridedCuda &imgs, const TensorDataStridedCuda &anchor,
                            ...
                            bool random, unsigned int seed, int rows, int cols, int channels, musaStream_t stream);
