...
namespace nvcv::musa { namespace osd {

#define PREALLOC_CMD_NUM 100

#define checkRuntime(call) check_runtime(call, #call, __LINE__, __FILE__)

inline static bool check_runtime(musaError_t e, const char *call, int line, const char *file)
{
    if (e != musaSuccess)
    {
...
public:
    ...
    void copy_host_to_device(musaStream_t stream = nullptr)
    {
        checkRuntime(musaMemcpyAsync(device_, host_, bytes(), musaMemcpyHostToDevice, stream));
    }

    void copy_device_to_host(musaStream_t stream = nullptr)
    {
        checkRuntime(musaMemcpyAsync(host_, device_, bytes(), musaMemcpyDeviceToHost, stream));
    }
    ...
    void alloc_or_resize_to(size_t size)
    {
        if (capacity_ < size)
        {
            ...
            checkRuntime(musaMallocHost(&host_, size * sizeof(T)));
            checkRuntime(musaMalloc(&device_, size * sizeof(T)));
            capacity_ = size;
        }
        size_ = size;
    }
    ...
    void free_memory()
    {
        if (host_ || device_)
        {
            checkRuntime(musaFreeHost(host_));
            checkRuntime(musaFree(device_));
            host_     = nullptr;
            device_   = nullptr;
            capacity_ = 0;
            size_     = 0;
