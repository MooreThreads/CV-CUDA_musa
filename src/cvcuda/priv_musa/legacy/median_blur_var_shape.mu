 ...
 */
template<typename T>
__device__ T fetch(T *shared, const musa::ImageBatchVarShapeWrapNHWC<T> src, int batchIdx, int h, int w, int c,
 ...
 */
template<typename T>
__global__ void median(const musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                       musa::Tensor1DWrap<const int2> ksize)
{
...
template<typename T>
__global__ void medianForSmallKernel(const musa::ImageBatchVarShapeWrapNHWC<T> src,
                                     musa::ImageBatchVarShapeWrapNHWC<T> dst, musa::Tensor1DWrap<const int2> ksize)
{
    int tx = threadIdx.x, ty = threadIdx.y;
    int blockX   = blockIdx.x * blockDim.x;
    int blockY   = blockIdx.y * blockDim.y;
...
void median(const ImageBatchVarShapeDataStridedCuda &in, const ImageBatchVarShapeDataStridedCuda &out,
            const TensorDataStridedCuda &ksize, int maxKHeight, int maxKWidth, musaStream_t stream)
{
    ...
    musa::ImageBatchVarShapeWrapNHWC<T> src(in, channels);
    musa::ImageBatchVarShapeWrapNHWC<T> dst(out, channels);
                                    ...
                                    const TensorDataStridedCuda &ksize, musaStream_t stream)
{
    ...
    checkCudaErrors(musaMemcpy2DAsync(m_kernelSizes.data(), sizeof(int) * 2, ksizeDataAccess->sampleData(0),
