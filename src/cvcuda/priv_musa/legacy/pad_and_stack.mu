...
nvcv::legacy::musa_op::ErrorCode padAndStackCaller(const ImageBatchVarShapeDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            const TensorDataStridedCuda &top, const TensorDataStridedCuda &left,
                            const float borderValue, musaStream_t stream)
{
    musa::BorderVarShapeWrap<const T, B> src(inData, musa::SetAll<T>(borderValue));

    auto topVec  = musa::CreateTensorWrapNHW<const int>(top);
    auto leftVec = musa::CreateTensorWrapNHW<const int>(left);
    ...
    if (outAccess->sampleStride() * outAccess->numSamples() <= musa::TypeTraits<int32_t>::max)
    {
    ...
    {
        LOG_ERROR("Output size exceeds " << musa::TypeTraits<int32_t>::max << ". Tensor is too large.");
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode padAndStack(const ImageBatchVarShapeDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                      const TensorDataStridedCuda &top, const TensorDataStridedCuda &left,
