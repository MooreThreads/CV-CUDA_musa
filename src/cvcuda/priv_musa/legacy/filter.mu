                         ...
                         int2 kernelAnchor)
{
    ...
    *dst.ptr(batch_idx, y, x) = musa::SaturateCast<T>(res);
}

template<typename T, NVCVBorderType B, class KernelWrapper>
nvcv::legacy::musa_op::ErrorCode Filter2DCaller(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
...
nvcv::legacy::musa_op::ErrorCode Filter2D(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData, KernelWrapper kernel,
...
constexpr musa::math::Vector<float, 9> kLaplacianKernel1{
    {0.0f,  1.0f, 0.0f,
     1.0f, -4.0f, 1.0f,
     0.0f,  1.0f, 0.0f}
};
constexpr musa::math::Vector<float, 9> kLaplacianKernel3{
    {2.0f,  0.0f, 2.0f,
     0.0f, -8.0f, 0.0f,
     2.0f,  0.0f, 2.0f}
};
...
ErrorCode Laplacian::infer(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData, int ksize,
                           float scale, NVCVBorderType borderMode, musaStream_t stream)
{
    if (!(ksize == 1 || ksize == 3))
    {
    ...
    musa_op::DataType  data_type   = GetLegacyDataType(inData.dtype());
    musa_op::DataShape input_shape = GetLegacyDataShape(inAccess->infoShape());
    ...
    typedef ErrorCode (*filter2D_t)(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                    musa::math::Vector<float, 9> kernel, Size2D kernelSize, int2 kernelAnchor,
    ...
    musa::math::Vector<float, 9> kernel;
...
{
    NVCV_CHECK_THROW(musaMalloc(&m_kernel, maxKernelSize.w * maxKernelSize.h * sizeof(float)));
}
                          ...
                          double2 sigma, NVCVBorderType borderMode, musaStream_t stream)
{
    ...
    if (kernelSize.w <= 0 && sigma.x > 0)
        kernelSize.w = nvcv::musa::round<int>(sigma.x * (data_type == kCV_8U ? 3 : 4) * 2 + 1) | 1;
    if (kernelSize.h <= 0 && sigma.y > 0)
        kernelSize.h = nvcv::musa::round<int>(sigma.y * (data_type == kCV_8U ? 3 : 4) * 2 + 1) | 1;
...
ErrorCode AverageBlur::infer(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                             Size2D kernelSize, int2 kernelAnchor, NVCVBorderType borderMode, musaStream_t stream)
{
    if (inData.dtype() != outData.dtype())
    {
        LOG_ERROR("Invalid DataType between input (" << inData.dtype() << ") and output (" << outData.dtype() << ")");
