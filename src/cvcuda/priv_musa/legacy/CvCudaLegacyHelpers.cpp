...
musa_op::DataFormat GetLegacyDataFormat(int32_t numberChannels, int32_t numberPlanes, int32_t numberInBatch)
{
    ...
    {
        return ((numberInBatch > 1) ? legacy::musa_op::DataFormat::kNHWC : legacy::musa_op::DataFormat::kHWC);
    }
    if (numberChannels == numberPlanes) //test for planar
    {
        return ((numberInBatch > 1) ? legacy::musa_op::DataFormat::kNCHW : legacy::musa_op::DataFormat::kCHW);
    }
...
static musa_op::DataType GetLegacyCvFloatType(int32_t bpc)
{
    if (bpc == 64)
        return musa_op::DataType::kCV_64F;
    if (bpc == 32)
        return musa_op::DataType::kCV_32F;
    if (bpc == 16)
        return musa_op::DataType::kCV_16F;

    throw Exception(Status::ERROR_INVALID_ARGUMENT, "Invalid bpc(%d) for float musa op type ", bpc);
}

static musa_op::DataType GetLegacyCvSignedType(int32_t bpc)
{
    if (bpc == 8)
        return musa_op::DataType::kCV_8S;
    if (bpc == 16)
        return musa_op::DataType::kCV_16S;
    if (bpc == 32)
        return musa_op::DataType::kCV_32S;

    throw Exception(Status::ERROR_INVALID_ARGUMENT, "Invalid bpc(%d) for signed musa op type ", bpc);
}

static musa_op::DataType GetLegacyCvUnsignedType(int32_t bpc)
{
    if (bpc == 8)
        return musa_op::DataType::kCV_8U;
    if (bpc == 16)
        return musa_op::DataType::kCV_16U;

    throw Exception(Status::ERROR_INVALID_ARGUMENT, "Invalid bpc(%d) for unsigned musa op type ", bpc);
}
...
{
    return musa_op::DataShape(shapeInfo.numSamples(), shapeInfo.numChannels(), shapeInfo.numRows(),
                              shapeInfo.numCols());
}
...
musa_op::DataFormat GetLegacyDataFormat(const ImageBatchVarShape &imgBatch)
{
    ...
    if (fmt.numPlanes() >= 2)
    {
        ...
        {
            return legacy::musa_op::DataFormat::kNCHW;
        }
        else
        {
            return legacy::musa_op::DataFormat::kCHW;
        }
    }
    else
    {
        if (imgBatch.numImages() >= 2)
        {
            return legacy::musa_op::DataFormat::kNHWC;
        }
        else
        {
            return legacy::musa_op::DataFormat::kHWC;
        }
    }
}
...
musa_op::DataFormat GetLegacyDataFormat(const TensorDataStridedCuda &container)
{
...
{
    using legacy::musa_op::ErrorCode;
...
const char *ToString(legacy::musa_op::ErrorCode err, const char **perrdescr)
{
