...
#include "gaussian_noise_util.muh"
...
__global__ void gaussian_noise_kernel(const Tensor3DWrap<T, StrideType> src, Tensor3DWrap<T, StrideType> dst,
                                      murandState *state, Tensor1DWrap<float, int32_t> mu,
                                      Tensor1DWrap<float, int32_t> sigma, int rows, int cols)
{
    ...
    murandState localState = state[id];
    while (offset < total_size)
    {
        int   dst_x                       = offset % cols;
        int   dst_y                       = offset / cols;
        float rand                        = murand_normal(&localState);
...
__global__ void gaussian_noise_per_channel_kernel(const Tensor4DWrap<T, StrideType> src,
                                                  Tensor4DWrap<T, StrideType> dst, murandState *state,
...
void gaussian_noise(const nvcv::TensorDataStridedCuda &d_in, const nvcv::TensorDataStridedCuda &d_out, int batch,
                    int rows, int cols, murandState *m_states, const nvcv::TensorDataStridedCuda &_mu,
                    const nvcv::TensorDataStridedCuda &_sigma, musaStream_t stream)
{
...
void gaussian_noise_per_channel(const nvcv::TensorDataStridedCuda &d_in, const nvcv::TensorDataStridedCuda &d_out,
                                int batch, int channels, int rows, int cols, murandState *m_states,
    ...
    , m_setupDone(false)
{
    ...
    musaError_t err = musaMalloc((void **)&m_states, sizeof(murandState) * BLOCK * maxBatchSize);
    if (err != musaSuccess)
    {
        LOG_ERROR("CUDA memory allocation error of size: " << sizeof(murandState) * BLOCK * maxBatchSize);
...
{
    musaError_t err = musaFree(m_states);
    if (err != musaSuccess)
                               ...
                               unsigned long long seed, musaStream_t stream)
{
    ...
    if (inMaxStride > musa::TypeTraits<int32_t>::max)
    {
        LOG_ERROR("Input size exceeds " << nvcv::musa::TypeTraits<int32_t>::max << ". Tensor is too large.");
    ...
    if (per_channel)
    {
        typedef void (*func_t)(const TensorDataStridedCuda &d_in, const TensorDataStridedCuda &d_out, int batch,
                               int channels, int rows, int cols, murandState *m_states, const TensorDataStridedCuda &mu,
                               const TensorDataStridedCuda &sigma, musaStream_t stream);
    ...
    {
        typedef void (*func_t)(const TensorDataStridedCuda &d_in, const TensorDataStridedCuda &d_out, int batch,
                               int rows, int cols, murandState *m_states, const TensorDataStridedCuda &mu,
                               const TensorDataStridedCuda &sigma, musaStream_t stream);
