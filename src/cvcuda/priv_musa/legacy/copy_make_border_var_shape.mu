...
__global__ void copyMakeBorderKernel(const SrcWrapper src, DstWrapper dst, const musa::Tensor3DWrap<int, int32_t> left_,
                                     const musa::Tensor3DWrap<int, int32_t> top_, int out_height, int out_width)
{
...
template<class SrcWrapper, class DstWrapper>
__global__ void copyMakeBorderKernel(const SrcWrapper src, DstWrapper dst, const musa::Tensor3DWrap<int, int32_t> left_,
                                     const musa::Tensor3DWrap<int, int32_t> top_)
{
    const int x         = blockIdx.x * blockDim.x + threadIdx.x;
    const int y         = blockIdx.y * blockDim.y + threadIdx.y;
    const int batch_idx = get_batch_idx();
...
{
    static void call(const ImageBatchVarShapeDataStridedCuda &src, musa::Tensor3DWrap<T> dst, const T &borderValue,
                     const musa::Tensor3DWrap<int, int32_t> &left, const musa::Tensor3DWrap<int, int32_t> &top,
                     int max_height, int max_width, musaStream_t stream)
    {
        dim3 blockSize(BLOCK, BLOCK / 4, 1);
        dim3 gridSize(divUp(max_width, blockSize.x), divUp(max_height, blockSize.y), src.numImages());

        musa::BorderVarShapeWrap<const T, B> brdSrc(src, borderValue);
    ...
    static void call(const ImageBatchVarShapeDataStridedCuda &src, musa::ImageBatchVarShapeWrap<T> dst,
                     const T &borderValue, const musa::Tensor3DWrap<int, int32_t> &left,
                     const musa::Tensor3DWrap<int, int32_t> &top, int max_height, int max_width, musaStream_t stream)
    {
        dim3 blockSize(BLOCK, BLOCK / 4, 1);
        dim3 gridSize(divUp(max_width, blockSize.x), divUp(max_height, blockSize.y), src.numImages());
...
void copyMakeBorder(const ImageBatchVarShapeDataStridedCuda &inData, const OutType &outData,
                    ...
                    const NVCVBorderType borderType, const float4 value, musaStream_t stream)
{
    typedef musa::MakeType<T, cn> src_type;
    src_type                      brdVal;
#pragma unroll
    for (int i = 0; i < cn; i++) musa::GetElement(brdVal, i) = musa::GetElement(value, i);

    musa::Tensor3DWrap<int, int32_t> topVec(top);
    musa::Tensor3DWrap<int, int32_t> leftVec(left);

    auto outSize = GetMaxImageSize(outData);

    using out_type =
        typename std::conditional<std::is_same<OutType, TensorDataStridedCuda>::value, musa::Tensor3DWrap<src_type>,
                                  musa::ImageBatchVarShapeWrap<src_type>>::type;
...
ErrorCode CopyMakeBorderVarShape::infer(const ImageBatchVarShapeDataStridedCuda &data_in,
                                        ...
                                        const float4 value, musaStream_t stream)
{
    return inferWarp(data_in, data_out, top, left, borderType, value, stream);
}
