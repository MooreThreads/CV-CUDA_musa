...
struct Convertor
{
    ...
    {
        return nvcv::musa::SaturateCast<DST_TYPE>(alpha * src + beta);
    }
};
...
nvcv::legacy::musa_op::ErrorCode convertToScaleCN(const nvcv::TensorDataStridedCuda &inData, const nvcv::TensorDataStridedCuda &outData,
                           const double alpha, const double beta, musaStream_t stream)
{
    ...
    using SRC_DATA_TYPE = nvcv::musa::MakeType<DT_SOURCE, NC>;
    using DST_DATA_TYPE = nvcv::musa::MakeType<DT_DEST, NC>;

    Convertor<SRC_DATA_TYPE, DST_DATA_TYPE, DT_AB> op;

    op.alpha = nvcv::musa::SaturateCast<DT_AB>(alpha);
    op.beta  = nvcv::musa::SaturateCast<DT_AB>(beta);
    ...
    {
        auto src = nvcv::musa::CreateTensorWrapNHW<SRC_DATA_TYPE, int32_t>(inData);
        auto dst = nvcv::musa::CreateTensorWrapNHW<DST_DATA_TYPE, int32_t>(outData);
...
nvcv::legacy::musa_op::ErrorCode convertToScale(const nvcv::TensorDataStridedCuda &inData, const nvcv::TensorDataStridedCuda &outData,
                         int numChannels, const double alpha, const double beta, musaStream_t stream)
{
    switch (numChannels)
    {
    case 1:
