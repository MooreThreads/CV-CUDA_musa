...
void center_crop(const nvcv::TensorDataStridedCuda &inData, const nvcv::TensorDataStridedCuda &outData, int crop_rows,
                 int crop_columns, const int batch_size, const int rows, const int columns, musaStream_t stream)
{
    ...
    auto src = nvcv::musa::CreateTensorWrapNHW<const T, StrideType>(inData);
    auto dst = nvcv::musa::CreateTensorWrapNHW<T, StrideType>(outData);
                            ...
                            int crop_columns, musaStream_t stream)
{
    ...
    if (std::max(inMaxStride, outMaxStride) > nvcv::musa::TypeTraits<int32_t>::max)
    {
