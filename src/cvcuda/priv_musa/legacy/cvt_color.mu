...
template<typename T, typename BT = nvcv::musa::BaseType<T>>
constexpr BT Alpha = std::is_floating_point_v<BT> ? 1 : nvcv::musa::TypeTraits<BT>::max;

namespace nvcv::legacy::musa_op {

template<typename T, typename StrideT>
using TensorWrap3D = nvcv::musa::Tensor3DWrap<T, StrideT>;

template<typename T, typename StrideT>
using TensorWrap4D = nvcv::musa::Tensor4DWrap<T, StrideT>;
                                  ...
                                  int batch_idx, int x, int y, int bidx)
{
    ...
    if constexpr (nvcv::musa::NumComponents<SrcT> == 4)
    {
        A = vec.w;
    }
    else
                                   ...
                                   int batch_idx, int x, int y, int bidx)
{
    ...
    vec.y = G;
    vec.z = bidx == 0 ? R : B;
    if constexpr (nvcv::musa::NumComponents<DstT> == 4)
    {
        vec.w = A;
    }
    *dst.ptr(batch_idx, y, x) = vec;
...
{
    using EltT = nvcv::musa::BaseType<SrcT>;
...
DEVICE_INLINE void bgr_to_yuv_int(T B_, T G_, T R_, T &Y_, T &Cb_, T &Cr_)
{
    ...
    constexpr int delta = ((T)(musa::TypeTraits<T>::max / 2 + 1)) << yuv_shift;
    ...
    Y_  = musa::SaturateCast<T>(Y);
    Cb_ = musa::SaturateCast<T>(Cb);
    Cr_ = musa::SaturateCast<T>(Cr);
}
...
DEVICE_INLINE void yuv_to_bgr_int(T Y_, T Cb_, T Cr_, T &B_, T &G_, T &R_)
{
    ...
    constexpr int delta = ((T)(musa::TypeTraits<T>::max / 2 + 1));
    ...
    B_ = musa::SaturateCast<T>(B);
    G_ = musa::SaturateCast<T>(G);
    R_ = musa::SaturateCast<T>(R);
}
...
DEVICE_INLINE void bgr_to_hsv_uchar(uchar b8, uchar g8, uchar r8, uchar &h8, uchar &s8, uchar &v8, bool isFullRange)
{
    ...
    const int vmin = musa::min(b, musa::min(g, r));
    const int v    = musa::max(b, musa::max(g, r));

    const int diff = v - vmin;
    const int vr   = v == r ? -1 : 0;
    const int vg   = v == g ? -1 : 0;

    const int hdiv_table = diff == 0 ? 0 : musa::SaturateCast<int>((hrange << hsv_shift) / (6.f * diff));
    const int sdiv_table = v == 0 ? 0 : musa::SaturateCast<int>((255 << hsv_shift) / (float)v);
    ...
    h8 = musa::SaturateCast<uint8_t>(h);
    s8 = (uint8_t)s;
    v8 = (uint8_t)v;
}
...
{
    float vmin = musa::min(r, musa::min(g, b));
    v          = musa::max(r, musa::max(g, b));
    float diff = v - vmin;
    ...
    color_conversion_common<Policy, 3, 3, EltT>(
        ...
        [isFullRange] __device__(const EltT(&r_HSV)[3], EltT(&r_BGR)[3])
        {
            if constexpr (std::is_same_v<EltT, uchar>)
            {
                ...
                r_BGR[0] = musa::SaturateCast<uchar>(Bf * 255.0f);
                r_BGR[1] = musa::SaturateCast<uchar>(Gf * 255.0f);
                r_BGR[2] = musa::SaturateCast<uchar>(Rf * 255.0f);
            }
            else
            {
...
DEVICE_INLINE void load_yuv420(const nvcv::musa::Tensor4DWrap<const EltT, StrideT> &src, EltT &Y, EltT &U, EltT &V,
...
{
    static_assert(std::is_same_v<nvcv::musa::BaseType<SrcT>, EltT>);
    color_conversion_common<Policy, 3, 3, EltT>(
        [&src, bidx] __device__(EltT(&r_BGR)[3], int batch_idx, int x, int y)
        {
            EltT A;
...
{
    static_assert(std::is_same_v<nvcv::musa::BaseType<DstT>, EltT>);
    color_conversion_common<Policy, 3, 3, EltT>(
...
inline nvcv::legacy::musa_op::ErrorCode Launch_BGR_to_RGB(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                   NVCVColorConversionCode code, musa_op::DataShape shape, int bidx,
                                   musaStream_t stream)
{
    ...
    auto srcWrap = musa::CreateTensorWrapNHW<const SrcT>(inData);
    auto dstWrap = musa::CreateTensorWrapNHW<DstT>(outData);
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_RGB(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            NVCVColorConversionCode code, musaStream_t stream)
{
    ...
    musa_op::DataType  inDataType = helpers::GetLegacyDataType(inData.dtype());
    musa_op::DataShape inputShape = helpers::GetLegacyDataShape(inAccess->infoShape());

    auto outAccess = TensorDataAccessStridedImagePlanar::Create(outData);
    NVCV_ASSERT(outAccess);

    musa_op::DataType  outDataType = helpers::GetLegacyDataType(outData.dtype());
    musa_op::DataShape outputShape = helpers::GetLegacyDataShape(outAccess->infoShape());
...
#define CVCUDA_BGR2RGB_CASE(T3, T4)       \
    ...
    else return nvcv::legacy::musa_op::ErrorCode::INVALID_DATA_SHAPE
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_GRAY_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                    musa_op::DataShape shape, musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode GRAY_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                             NVCVColorConversionCode code, musaStream_t stream)
{
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_BGR_to_GRAY(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                    musa_op::DataShape shape, int bidx, musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_GRAY(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                             NVCVColorConversionCode code, musaStream_t stream)
{
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_BGR_to_YUV(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                   musa_op::DataShape shape, int bidx, musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_YUV(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            NVCVColorConversionCode code, musaStream_t stream)
{
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_YUV_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                   musa_op::DataShape shape, int bidx, musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
...
inline nvcv::legacy::musa_op::ErrorCode YUV_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            NVCVColorConversionCode code, musaStream_t stream)
{
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_BGR_to_HSV(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                   musa_op::DataShape shape, int bidx, bool isFullRange, bool strides_64b,
                                   musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
    ...
    if (strides_64b)
    {
        auto srcWrap = musa::CreateTensorWrapNHW<const SrcT, int64_t>(inData);
        auto dstWrap = musa::CreateTensorWrapNHW<DstT, int64_t>(outData);
        bgr_to_hsv_nhwc<Policy><<<gridSize, blockSize, 0, stream>>>(srcWrap, dstWrap, dstSize, bidx, isFullRange);
    }
    else
    {
        auto srcWrap = musa::CreateTensorWrapNHW<const SrcT, int32_t>(inData);
        auto dstWrap = musa::CreateTensorWrapNHW<DstT, int32_t>(outData);
        bgr_to_hsv_nhwc<Policy><<<gridSize, blockSize, 0, stream>>>(srcWrap, dstWrap, dstSize, bidx, isFullRange);
    }
    checkKernelErrors();
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_HSV(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            NVCVColorConversionCode code, musaStream_t stream)
{
    ...
    const bool strides_64b = std::max(inAccess->sampleStride() * inAccess->numSamples(),
                           ...
                           > nvcv::musa::TypeTraits<int32_t>::max;
...
template<typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_HSV_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                   musa_op::DataShape shape, int bidx, bool isFullRange, bool strides_64b,
                                   musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
...
inline nvcv::legacy::musa_op::ErrorCode HSV_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                            NVCVColorConversionCode code, musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode Launch_YUV420xp_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                        musa_op::DataShape shape, int bidx, int uidx, bool strides_64b,
                                        musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
    ...
    if (strides_64b)
    {
        ...
        auto srcWrap = musa::CreateTensorWrapNHWC<const SrcT, int64_t>(inData);
    ...
    {
        // YUV420 input: 4D tensor with scalar type.
        auto srcWrap = musa::CreateTensorWrapNHWC<const SrcT, int32_t>(inData);
        // BGR output: 3D tensor with vector type.
        auto dstWrap = musa::CreateTensorWrapNHW<DstT, int32_t>(outData);
        yuv420_to_bgr_char_nhwc<IsSemiPlanar, Policy>
            <<<gridSize, blockSize, 0, stream>>>(srcWrap, dstWrap, dstSize, bidx, uidx);
...
inline nvcv::legacy::musa_op::ErrorCode YUV420xp_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                 NVCVColorConversionCode code, musaStream_t stream)
{
    int bidx
    ...
    case NVCV_COLOR_YUV2GRAY_420:
    {
        ...
        for (int i = 0; i < inputShape.N; i++)
        {
            ...
            checkCudaErrors(musaMemcpy2DAsync(dstPtr, dpitch, srcPtr, spitch, rgb_width, rgb_height,
...
template<bool IsSemiPlanar, typename SrcT, typename DstT>
inline nvcv::legacy::musa_op::ErrorCode Launch_BGR_to_YUV420xp(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                        DataShape inputShape, int bidx, int uidx, bool strides_64b, musaStream_t stream)
{
    using Policy = CvtKernelPolicy<32, 4, 4>;
    ...
    if (strides_64b)
    {
        auto srcWrap = musa::CreateTensorWrapNHW<const SrcT, int64_t>(inData); // RGB input: 3D tensor with vector type.
        auto dstWrap = musa::CreateTensorWrapNHWC<DstT, int64_t>(outData);     // YUV420 output: 4D scalar tensor.
    ...
    {
        auto srcWrap = musa::CreateTensorWrapNHW<const SrcT, int32_t>(inData); // RGB input: 3D tensor with vector type.
        auto dstWrap = musa::CreateTensorWrapNHWC<DstT, int32_t>(outData);     // YUV420 output: 4D scalar tensor.
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_YUV420xp(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                 NVCVColorConversionCode code, musaStream_t stream)
{
    int bidx
...
inline nvcv::legacy::musa_op::ErrorCode YUV422_to_BGR(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                               NVCVColorConversionCode code, musaStream_t stream)
{
    int bidx
    ...
    auto srcWrap = musa::CreateTensorWrapNHWC<uint8_t>(inData);
    auto dstWrap = musa::CreateTensorWrapNHWC<uint8_t>(outData);
