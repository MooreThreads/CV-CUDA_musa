...
#pragma unroll
    for (int k = 0; k < X_STEPS; ++k)
    {
        ...
        uchar t = cmp(s[(out_y - shift_y) * s_width + out_x - shift_x] + idelta, musa::SaturateCast<uchar>(res))
                    ? maxValue
                    : 0;
...
nvcv::legacy::musa_op::ErrorCode adaptive_threshold_caller(const TensorDataStridedCuda &in, const TensorDataStridedCuda &out,
                                    ...
                                    musaStream_t stream)
{
    ...
    {
        auto src = musa::CreateBorderWrapNHW<const T, B, int32_t>(in, musa::SetAll<T>(0.f));
        auto dst = musa::CreateTensorWrapNHW<T, int32_t>(out);
    ...
    : CudaBaseOp(maxInputShape, maxOutputShape)
{
    ...
    NVCV_CHECK_THROW(musaMalloc(&m_kernel, bufferSize));
}
...
ErrorCode AdaptiveThreshold::infer(const TensorDataStridedCuda &in, const TensorDataStridedCuda &out,
                                   ...
                                   musaStream_t stream)
{
    musa_op::DataFormat input_format  = GetLegacyDataFormat(in.layout());
    musa_op::DataFormat output_format = GetLegacyDataFormat(out.layout());
    ...
    musa_op::DataType  data_type   = GetLegacyDataType(in.dtype());
    ...
    uchar imaxval = musa::SaturateCast<uchar>(maxValue);
