                                          ...
                                          int *v_kk_offset, work_type init_buffer, bool round_up, bool use_share_mem)
{
    ...
    if (dst_x < out_width && dst_y < out_height)
    {
        ...
        for (int c = 0; c < src.nch; ++c)
        {
            ...
            if (round_up)
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T2>(std::round(h_ss));
            else
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T2>(h_ss);
        }
    }
}
                                        ...
                                        int *v_kk_offset, work_type init_buffer, bool round_up, bool use_share_mem)
{
    ...
    if (dst_x < out_width && dst_y < out_height)
    {
        ...
        for (int c = 0; c < src.ch; ++c)
        {
            ...
            if (round_up)
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T2>(std::round(ss));
            else
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T2>(ss);
        }
    }
}
                             ...
                             musaStream_t stream)
{
    if (ws.hostMem.ready != nullptr)
        checkCudaErrors(musaEventSynchronize(ws.hostMem.ready));
    ...
    checkCudaErrors(musaMemcpyAsync((void *)gpu_workspace, (void *)cpu_workspace, current_buffer_size,
                                    musaMemcpyHostToDevice, stream));

    if (ws.hostMem.ready != nullptr)
        checkCudaErrors(musaEventRecord(ws.hostMem.ready, stream));
...
void pillow_resize_filter_var_shape(const ImageBatchVarShape &inData, const ImageBatchVarShape &outData,
                                    const Workspace &ws, NVCVInterpolationType interpolation, musaStream_t stream)
{
                                                                     ...
                                                                     DataShape max_output_shape, DataType max_data_type)
{
    ...
    req.musaMem.size      = buffer_size;
    req.musaMem.alignment = kDefaultDeviceAlignment;
