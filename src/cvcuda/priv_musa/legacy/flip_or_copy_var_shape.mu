...
template<typename T>
__global__ void flip_kernel(const musa::ImageBatchVarShapeWrap<T> src, musa::ImageBatchVarShapeWrap<T> dst,
                            const musa::Tensor1DWrap<int> flipCode)
{
          ...
          const TensorDataStridedCuda &flipCode, musaStream_t stream)
{
    ...
    musa::ImageBatchVarShapeWrap<T> src(input);
    musa::ImageBatchVarShapeWrap<T> dst(output);
    musa::Tensor1DWrap<int>         flip_code(flipCode);
