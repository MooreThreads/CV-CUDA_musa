...
#include "cub/cub.cuh"
#include "inpaint_utils.muh"
#include "reduce_kernel_utils.muh"
...
{
    using work_type = nvcv::musa::ConvertBaseTypeTo<T, D>;
    work_type res   = nvcv::musa::SetAll<T>(maxmin);
...
void MorphFilter2DCaller(Ptr2dNHWC<T> src, Ptr2dNHWC<T> dst, const unsigned char *kernel, int kWidth, int kHeight,
                         int anchorX, int anchorY, musaStream_t stream)
{
    dim3 block(16, 16);
    dim3 grid(divUp(dst.cols, block.x), divUp(dst.rows, block.y), dst.batches);

    T               val = std::numeric_limits<T>::min();
    BrdReplicate<T> brd(dst.rows, dst.cols, nvcv::musa::SetAll<T>(val)); // set all value as val
...
template<typename T>
void sub_helper(Ptr2dNHWC<T> src1, Ptr2dNHWC<T> src2, Ptr2dNHWC<T> dst, musaStream_t stream)
{
    dim3 block(16, 16);
    dim3 grid(divUp(dst.cols, block.x), divUp(dst.rows, block.y), dst.batches);
...
template<typename T>
void set_border_helper(Ptr2dNHWC<T> src, T value, musaStream_t stream)
{
    dim3 block(16, 1);
...
template<typename T, typename T2>
void set_value(Ptr2dNHWC<T> src, Ptr2dNHWC<T2> mask, T value, musaStream_t stream)
{
    dim3 block(16, 16);
...
inline int finish_flag_reduce(Ptr2dNHWC<unsigned char> src_ptr, int *d_out, int *d_out2, musaStream_t stream)
{
    ...
    checkKernelErrors();
    int rst = 0;
    checkCudaErrors(musaMemcpyAsync((void *)&rst, (void *)d_out2, sizeof(int), musaMemcpyDeviceToHost, stream));
    checkCudaErrors(musaStreamSynchronize(stream));
    return rst;
}

template<typename T>
nvcv::legacy::musa_op::ErrorCode inpaint_helper(const nvcv::TensorDataStridedCuda &inData, const nvcv::TensorDataStridedCuda &mask,
                         ...
                         musaStream_t stream)
{
    ...
    for (uint32_t i = 0; i < inAccess->numSamples(); ++i)
    {
        ...
        checkCudaErrors(musaMemcpy2DAsync(outSampData, outAccess->rowStride(), inSampData, inAccess->rowStride(),
    ...
    checkCudaErrors(musaMemsetAsync(inpaint_mask_ptr, KNOWN, sizeof(unsigned char) * batch * erows * ecols * 1,
    ...
    if (maskAccess->sampleStride() * batch <= nvcv::musa::TypeTraits<int32_t>::max)
    {
    ...
    {
        LOG_ERROR("Mask size exceeds " << nvcv::musa::TypeTraits<int32_t>::max << ". Tensor is too large.");
    ...
    checkCudaErrors(musaMemsetAsync(f_ptr, KNOWN, sizeof(unsigned char) * batch * erows * ecols * 1,
                                    stream)); // cvSet(f,cvScalar(KNOWN,0,0,0));
    checkCudaErrors(musaMemsetAsync(t_ptr, 1.0e6f, sizeof(float) * batch * erows * ecols * 1,
    ...
    if (outAccess->sampleStride() * batch <= nvcv::musa::TypeTraits<int32_t>::max)
    {
    ...
    {
        LOG_ERROR("Output size exceeds " << nvcv::musa::TypeTraits<int32_t>::max << ". Tensor is too large.");
        return nvcv::legacy::musa_op::ErrorCode::INVALID_PARAMETER;
    }
...
Inpaint::Inpaint(DataShape max_input_shape, DataShape max_output_shape, int maxBatchSize, Size2D maxShape)
    ...
    , m_workspace(nullptr)
{
    musaError_t err = musaMalloc(&m_kernel_ptr, sizeof(unsigned char) * maxBatchSize * 3 * 3);
    if (err != musaSuccess)
    {
    ...
    err = musaMalloc(&m_workspace, buffersize);
    if (err != musaSuccess)
    {
        musaFree(m_kernel_ptr);
...
Inpaint::~Inpaint()
{
    musaError_t err0 = musaFree(m_kernel_ptr);
    musaError_t err1 = musaFree(m_workspace);
    if (err0 != musaSuccess || err1 != musaSuccess)
    {
                         ...
                         const TensorDataStridedCuda &outData, double inpaintRadius, musaStream_t stream)
{
    ...
    typedef ErrorCode (*inpaint_t)(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &mask,
                                   ...
                                   int maxBatchSize, musaStream_t stream);
