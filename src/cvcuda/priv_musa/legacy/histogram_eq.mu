...
__global__ void prefix_sum_with_norm_kernel(CdfWrapper histogram, nvcv::Size2D dstSize)
{
    ...
    else
    {
        ...
        histogram[coordHisto]
            = nvcv::musa::SaturateCast<int>(1.0 * temp[pout * 256 + tid] / temp[pout * 256 + 255] * 255);
    }
}
...

{
    ...
    if (src_x < dstSize.w && src_y < dstSize.h)
    {
        ...
        for (int ch = 0; ch < channels; ch++)
        {
            ...
            dst[coordImg] = nvcv::musa::SaturateCast<uchar>((temp[src[coordImg] + offset]));
        }
    }
}
...
HistogramEq::HistogramEq(int maxBatchSize)
{
    ...
    {
        NVCV_CHECK_THROW(musaMalloc(&m_histoArray, m_sizeOfHisto));
    }
}
...
HistogramEq::~HistogramEq()
{
    if (m_histoArray)
    {
        musaFree(m_histoArray);
        m_histoArray = nullptr;
    }
}

template<typename SrcWrap, typename DstWrap, typename HistWrap>
nvcv::legacy::musa_op::ErrorCode infer_histogram(SrcWrap src, DstWrap dst, HistWrap histo, int batch, nvcv::Size2D dstSize, int channels,
                          musaStream_t stream)
{
    {
                             ...
                             musaStream_t stream)
{
    ...
    //clear the histogram.
    checkCudaErrors(musaMemsetAsync(m_histoArray, 0, m_sizeOfHisto, stream));

    // 2d wrap since its an array of [256 * channels] = width, height = samples
    auto histo = nvcv::musa::Tensor2DWrap<int, int32_t>(m_histoArray, (int)(256 * channels * sizeof(int)));
    ...
    {
        auto src = nvcv::musa::CreateTensorWrapNHWC<uchar, int32_t>(inData);
        auto dst = nvcv::musa::CreateTensorWrapNHWC<uchar, int32_t>(outData);
