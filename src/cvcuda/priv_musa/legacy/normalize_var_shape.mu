...
template<typename T>
__device__ __forceinline__ void storeMusaValue(T *dst, T value)
{
    if constexpr (musa::NumElements<T> == 1)
    {
        *dst = value;
    }
    else
    {
        ...
        auto *out = reinterpret_cast<BT *>(dst);
        out[0]    = musa::GetElement<0>(value);
        if constexpr (musa::NumElements<T> >= 2)
            out[1] = musa::GetElement<1>(value);
        if constexpr (musa::NumElements<T> >= 3)
            out[2] = musa::GetElement<2>(value);
        if constexpr (musa::NumElements<T> == 4)
            out[3] = musa::GetElement<3>(value);
    }
}
...
__global__ void normKernel(const musa::ImageBatchVarShapeWrap<const T> src, musa::ImageBatchVarShapeWrap<out_T> dst,
                           const scale_type *scale, const base_type *base, float global_scale, float global_shift)
{
    ...
    storeMusaValue(dst.ptr(batch_idx, dst_y, dst_x),
                   musa::SaturateCast<out_T>((out - *base) * *scale * global_scale + global_shift));
}

// (float3 - float3) * float3 / (float3 - float) * float3 / (float3 - float3) * float / (float3 - float) * float
template<typename T, typename out_T, typename base_type, typename scale_type>
__global__ void normInvStdDevKernel(const musa::ImageBatchVarShapeWrap<const T> src,
                                    musa::ImageBatchVarShapeWrap<out_T> dst, const scale_type *scale,
                                    const base_type *base, float global_scale, float global_shift, float epsilon)
{
    ...
    scale_type mul = 1.0f / musa::sqrt(x);

    T out = *src.ptr(batch_idx, dst_y, dst_x);
    storeMusaValue(dst.ptr(batch_idx, dst_y, dst_x),
                   musa::SaturateCast<out_T>((out - *base) * mul * global_scale + global_shift));
}

template<typename T, typename out_T, typename base_type, typename scale_type>
void normWrap(const ImageBatchVarShapeDataStridedCuda &in, const base_type *base, const scale_type *scale,
              const ImageBatchVarShapeDataStridedCuda &out, float global_scale, float shift, musaStream_t stream)
{
    ...
    musa::ImageBatchVarShapeWrap<out_T>   dst_ptr(out);
...
void norm(const ImageBatchVarShapeDataStridedCuda &in, const TensorDataAccessStridedImagePlanar &base,
          ...
          float global_scale, float shift, musaStream_t stream)
{
...
void normInvStdDev(const ImageBatchVarShapeDataStridedCuda &in, const TensorDataAccessStridedImagePlanar &base,
                   const TensorDataAccessStridedImagePlanar &scale, const ImageBatchVarShapeDataStridedCuda &out,
                   float global_scale, float shift, float epsilon, musaStream_t stream)
{
    using work_type = musa::ConvertBaseTypeTo<float, T>;
    if (base.numChannels() != 1 && scale.numChannels() != 1)
    {
...
ErrorCode NormalizeVarShape::infer(const nvcv::ImageBatchVarShapeDataStridedCuda &inData,
                                   ...
                                   const float shift, const float epsilon, const uint32_t flags, musaStream_t stream)
{
