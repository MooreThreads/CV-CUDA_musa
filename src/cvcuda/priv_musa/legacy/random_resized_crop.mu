                                  ...
                                  const int *left_, const float *scale_x_, const float *scale_y_)
{
    ...
    { //generic copy pixel to pixel
        ...
        const int sx = musa::min(__float2int_rd((dst_x + 0.5f) * scale_x) + left, srcSize.x - 1);
        const int sy = musa::min(__float2int_rd((dst_y + 0.5f) * scale_y) + top, srcSize.y - 1);
...
void resize(const SrcWrapper &src, const DstWrapper &dst, const NVCVInterpolationType interpolation,
            musaStream_t stream, const int *top, const int *left, const float *scale_x, const float *scale_y,
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode resize(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                 const NVCVInterpolationType interpolation, musaStream_t stream, const int *top, const int *left,
                 const float *scale_x, const float *scale_y)
{
    ...
    {
        auto src = musa::CreateTensorWrapNHW<T, int32_t>(inData);
    ...
    , m_maxBatchSize(maxBatchSize)
{
    ...
    if (maxBatchSize > 0)
    {
        ...
        NVCV_CHECK_LOG(musaMalloc(&m_gpuCropParams, bufferSize));
...
{
    NVCV_CHECK_LOG(musaFree(m_gpuCropParams));
                                   ...
                                   const NVCVInterpolationType interpolation, musaStream_t stream)
{
    ...
    checkCudaErrors(
        musaMemcpyAsync((void *)m_gpuCropParams, (void *)m_cpuCropParams, buffer_size, musaMemcpyHostToDevice, stream));

    typedef ErrorCode (*func_t)(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                const NVCVInterpolationType interpolation, musaStream_t stream, const int *top,
