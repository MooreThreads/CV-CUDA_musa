...
__global__ void horizontal_pass(const musa_op::Ptr2dNHWC<T> src, musa_op::Ptr2dNHWC<T> dst, NVCVRectI roi,
                                ...
                                bool use_share_mem)
{
    ...
    if (dst_x < out_width && dst_y < out_height)
    {
        ...
        for (int c = 0; c < src.ch; ++c)
        {
            ...
            if (round_up)
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T>(std::round(h_ss));
            else
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T>(h_ss);
        }
    }
}

template<class T, class Filter>
__global__ void vertical_pass(const musa_op::Ptr2dNHWC<T> src, musa_op::Ptr2dNHWC<T> dst, NVCVRectI roi,
                              ...
                              work_type *v_kk, work_type init_buffer, bool round_up, bool use_share_mem)
{
    ...
    if (dst_x < out_width && dst_y < out_height)
    {
        ...
        for (int c = 0; c < src.ch; ++c)
        {
            ...
            if (round_up)
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T>(std::round(ss));
            else
                *dst.ptr(batch_idx, dst_y, dst_x, c) = musa::SaturateCast<T>(ss);
        }
    }
}
...
void pillow_resize_v2(const TensorDataAccessStridedImagePlanar &inData,
                      ...
                      work_type init_buffer, bool round_up, musaStream_t stream)
{
    musa_op::DataShape   input_shape = GetLegacyDataShape(inData.infoShape());
...
void pillow_resize_filter(const TensorDataAccessStridedImagePlanar &inData,
                          ...
                          NVCVInterpolationType interpolation, musaStream_t stream)
{
                                                             ...
                                                             DataType max_data_type)
{
    ...
    req.musaMem = {size, 256};
    return req;
}

ErrorCode PillowResize::infer(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                              const NVCVInterpolationType interpolation, musaStream_t stream, const Workspace &ws)
{
    ...
    if (ws.musaMem.ready != nullptr)
        checkCudaErrors(musaStreamWaitEvent(stream, ws.musaMem.ready));

    void *gpu_workspace = ws.musaMem.data;
    ...
    if (ws.musaMem.ready != nullptr)
        checkCudaErrors(musaEventRecord(ws.musaMem.ready, stream));
