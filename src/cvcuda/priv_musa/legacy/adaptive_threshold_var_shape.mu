...
template<typename CMP, musa::RoundMode RM, typename SrcWrapper, typename DstWrapper>
__global__ void adaptive_threshold(const SrcWrapper src, DstWrapper dst,
                                   musa::Tensor1DWrap<double, int32_t> maxValueArr,
                                   musa::Tensor1DWrap<int, int32_t>    blockSizeArr,
                                   musa::Tensor1DWrap<double, int32_t> cArr, musa::Tensor3DWrap<float, int32_t> kernel)
{
    ...
    if (out_x >= out_width || out_y >= out_height)
        return;
    CMP         cmp;
    const uchar maxv  = musa::SaturateCast<uchar>(maxValueArr[batch_idx]);
    const int   delta = musa::round<RM>(cArr[batch_idx]);
#pragma unroll
    for (int k = 0; k < X_STEPS; ++k)
    {
        ...
        uchar t
            = cmp(s[(out_y - shift_y) * s_width + out_x - shift_x] + delta, musa::SaturateCast<uchar>(res)) ? maxv : 0;
...
void adaptive_threshold_caller(const ImageBatchVarShapeDataStridedCuda &in,
                               ...
                               musa::Tensor1DWrap<int, int32_t> blockSizeArr, musa::Tensor1DWrap<double, int32_t> cArr,
                               musa::Tensor3DWrap<float, int32_t> kernel, int maxBlockSize, musaStream_t stream)
{
    float                                borderValue = .0f;
    musa::BorderVarShapeWrap<const D, B> src(in, musa::SetAll<D>(borderValue));
    musa::ImageBatchVarShapeWrap<D>      dst(out);
    ...
    {
        adaptive_threshold<CMP, musa::RoundMode::UP>
            <<<grid, block, s_mem_size, stream>>>(src, dst, maxValueArr, blockSizeArr, cArr, kernel);
    }
    else
    {
        adaptive_threshold<CMP, musa::RoundMode::DOWN>
            <<<grid, block, s_mem_size, stream>>>(src, dst, maxValueArr, blockSizeArr, cArr, kernel);
    }
    ...
    , m_maxBatchSize(maxVarShapeBatchSize)
{
    ...
    if (maxVarShapeBatchSize > 0)
    {
        ...
        NVCV_CHECK_LOG(musaMalloc(&m_kernel, bufferSize));
    }
}
                                           ...
                                           musaStream_t stream)
{
    ...
    musa::Tensor1DWrap<double, int32_t> maxValueArr(maxValue);
    musa::Tensor1DWrap<int, int32_t>    blockSizeArr(blockSize);
    musa::Tensor1DWrap<double, int32_t> cArr(c);
    ...
    musa::Tensor3DWrap<float, int32_t> kernelTensor(kernelPtr, kernelPitch1, kernelPitch2);
