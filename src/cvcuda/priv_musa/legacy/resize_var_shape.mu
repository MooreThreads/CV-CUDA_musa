...
inline __device__ T *_cacheAlignedBufferedReadVS(musa::ImageBatchVarShapeWrap<const T> srcImage, int width,
...
template<typename T>
__global__ void resize_NN(musa::ImageBatchVarShapeWrap<const T> src, musa::ImageBatchVarShapeWrap<T> dst)
{
    ...
    { //generic copy pixel to pixel
        ...
        const int   sx      = musa::min(__float2int_rd((dst_x + 0.5f) * scale_x), width - 1);
        const int   sy      = musa::min(__float2int_rd((dst_y + 0.5f) * scale_y), height - 1);
...
template<typename T>
__global__ void resize_bilinear(musa::ImageBatchVarShapeWrap<const T> src, musa::ImageBatchVarShapeWrap<T> dst)
{
    const int dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    const int dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    const int batch_idx = get_batch_idx();
    ...
    if ((dst_x < dstWidth) && (dst_y < dstHeight))
    {
        ...
        using work_type = musa::ConvertBaseTypeTo<float, T>;

        //y coordinate
        float fy = (float)((dst_y + 0.5f) * scale_y - 0.5f);
        int   sy = musa::round<musa::RoundMode::DOWN, int>(fy);

        fy = ((sy < 0) ? 0 : ((sy > height - 2) ? 1 : fy - sy));
        sy = musa::max(0, musa::min(sy, height - 2));
        ...
        { //compute source data position and weight for [x0] components
            ...
            int   sx = musa::round<musa::RoundMode::DOWN, int>(fx);

            fx = ((sx < 0) ? 0 : ((sx > width - 2) ? 1 : fx - sx));
            sx = musa::max(0, musa::min(sx, width - 2));

            *dst.ptr(batch_idx, dst_y, dst_x)
                = musa::SaturateCast<T>((1.0f - fx) * (aPtr[sx] * (1.0f - fy) + bPtr[sx] * fy)
...
template<typename T>
__global__ void resize_bicubic(musa::ImageBatchVarShapeWrap<const T> src, musa::ImageBatchVarShapeWrap<T> dst)
{ //optimized for aligned read
    ...
    if ((dst_x < dstWidth) & (dst_y < dstHeight))
    {
        ...
        int   sy = musa::round<musa::RoundMode::DOWN, int>(fy);
        fy -= sy;
        sy = musa::max(1, musa::min(sy, height - 3));
        ...
        work_type accum = musa::SetAll<work_type>(0);

        float fx = (float)((dst_x + 0.5f) * scale_x - 0.5f);
        int   sx = musa::round<musa::RoundMode::DOWN, int>(fx);
        fx -= sx;
        fx *= ((sx >= 1) && (sx < width - 3));
        sx = musa::max(1, musa::min(sx, width - 3));
...
#pragma unroll
        ...
        //correct math
        *dst.ptr(batch_idx, dst_y, dst_x) = musa::SaturateCast<T>(accum);
#else
        //abs() needed to match legacy operator.
        *dst.ptr(batch_idx, dst_y, dst_x) = musa::SaturateCast<T>(musa::abs(accum));
#endif
    }
} //resize_bicubic
...
template<typename T>
__global__ void resize_area_ocv_align(const musa::ImageBatchVarShapeWrap<const T>                   src,
                                      const musa::BorderVarShapeWrap<const T, NVCV_BORDER_CONSTANT> brd_src,
                                      musa::ImageBatchVarShapeWrap<T>                               dst)
{
    ...
    int    iscale_x     = musa::SaturateCast<int>(scale_x);
    int    iscale_y     = musa::SaturateCast<int>(scale_y);
    ...
    if (scale_x >= 1.0f && scale_y >= 1.0f) // zoom out
    {
        if (is_area_fast) // integer multiples
        {
            ...
            int sx1 = musa::round<musa::RoundMode::UP, int>(fsx1);
            int sx2 = musa::round<musa::RoundMode::DOWN, int>(fsx2);

            float fsy1 = y * scale_y;
            float fsy2 = fsy1 + scale_y;

            int sy1 = musa::round<musa::RoundMode::UP, int>(fsy1);
            int sy2 = musa::round<musa::RoundMode::DOWN, int>(fsy2);
            ...
            *dst.ptr(batch_idx, y, x) = musa::SaturateCast<T>(out);
            return;
        }
    ...
    int   sy = musa::round<musa::RoundMode::DOWN, int>(y * scale_y);
    float fy = (float)((y + 1) - (sy + 1) * inv_scale_y);
    fy       = fy <= 0 ? 0.f : fy - musa::round<musa::RoundMode::DOWN, int>(fy);

    float cbufy[2];
    cbufy[0] = 1.f - fy;
    cbufy[1] = fy;

    int   sx = musa::round<musa::RoundMode::DOWN, int>(x * scale_x);
    float fx = (float)((x + 1) - (sx + 1) * inv_scale_x);
    fx       = fx < 0 ? 0.f : fx - musa::round<musa::RoundMode::DOWN, int>(fx);
    ...
    *dst.ptr(batch_idx, y, x) = musa::SaturateCast<T>((*src.ptr(batch_idx, sy, sx) * cbufx[0] * cbufy[0]
...
void resize(const ImageBatchVarShapeDataStridedCuda &in, const ImageBatchVarShapeDataStridedCuda &out,
            const int interpolation, musaStream_t stream)
{
    NVCV_ASSERT(in.numImages() == out.numImages());

    musa::ImageBatchVarShapeWrap<const T> src_ptr(in);
    ...
    case NVCV_INTERP_AREA:
        musa::BorderVarShapeWrap<const T, NVCV_BORDER_CONSTANT> brdSrc(in);
