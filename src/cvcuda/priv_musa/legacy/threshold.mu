                                ...
                                Tensor1DWrap<double, int32_t> _maxval, int height, int width, int channel)
{
    ...
    T      maxval  = nvcv::musa::SaturateCast<T>(imaxval);
...
nvcv::legacy::musa_op::ErrorCode thresholdDispatch(const nvcv::TensorDataStridedCuda &input, const nvcv::TensorDataStridedCuda &output,
                            ...
                            musaStream_t stream)
{
    ...
    using vectype    = nvcv::musa::MakeType<T, N>;
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode thresholdScale(const nvcv::TensorDataStridedCuda &input, const nvcv::TensorDataStridedCuda &output,
...
static void getThreshVal_Triangle(const nvcv::TensorDataStridedCuda &inData,
                                  ...
                                  int batch, musaStream_t stream)
{
    checkCudaErrors(musaMemsetAsync(histogram, 0, sizeof(int) * 256 * batch, stream));
...
static void getThreshVal_Otsu(const nvcv::TensorDataStridedCuda &inData, const nvcv::TensorDataStridedCuda &threshold,
                              int *histogram, int rows, int cols, int batch, musaStream_t stream)
{
    int size = rows * cols;
    checkCudaErrors(musaMemsetAsync(histogram, 0, sizeof(int) * 256 * batch, stream));
    ...
    , m_type(type)
{
    ...
    {
        musaError_t err = musaMalloc(&m_histogram, sizeof(int) * 256 * maxBatchSize);
        if (err != musaSuccess)
        {
...
Threshold::~Threshold()
{
    if (m_automatic_thresh != 0)
    {
        musaError_t err = musaFree(m_histogram);
        if (err != musaSuccess)
                           ...
                           musaStream_t stream)
{
    ...
    typedef ErrorCode (*threshold_t)(const TensorDataStridedCuda &input, const TensorDataStridedCuda &output,
                                     ...
                                     DataType data_type, musaStream_t stream);
