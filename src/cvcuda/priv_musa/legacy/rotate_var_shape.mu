...
__global__ void compute_warpAffine(const int numImages, const musa::Tensor1DWrap<double> angleDeg,
                                   const musa::Tensor2DWrap<double> shift, double *d_aCoeffs)
{
...
void rotate(const ImageBatchVarShapeDataStridedCuda &in, const ImageBatchVarShapeDataStridedCuda &out,
            double *d_aCoeffs, musaStream_t stream)
{
    ...
    musa::InterpolationVarShapeWrap<const T, NVCV_BORDER_REPLICATE, I> src(in);
...
void rotate(const ImageBatchVarShapeDataStridedCuda &in, const ImageBatchVarShapeDataStridedCuda &out,
            double *d_aCoeffs, const NVCVInterpolationType interpolation, musaStream_t stream)
{
    ...
    , m_maxBatchSize(maxBatchSize)
{
    if (m_maxBatchSize > 0)
    {
        ...
        musaError_t err        = musaMalloc(&d_aCoeffs, bufferSize);
        if (err != musaSuccess)
        {
...
RotateVarShape::~RotateVarShape()
{
    ...
    {
        musaError_t err = musaFree(d_aCoeffs);
        if (err != musaSuccess)
        {
                                ...
                                musaStream_t stream)
{
    ...
    musa::Tensor1DWrap<double> angleDecPtr(angleDeg);
    musa::Tensor2DWrap<double> shiftPtr(shift);
