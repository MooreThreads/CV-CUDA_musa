                           ...
                           const TensorDataStridedCuda &histogram, musaStream_t stream)
{
    ...
    checkCudaErrors(musaMemset2DAsync(histoAccess->sampleData(0), histoAccess->rowStride(), 0, histoAccess->numCols(),
                                      histoAccess->numRows(), stream));

    auto src   = nvcv::musa::CreateTensorWrapNHW<uchar>(inData);
    auto histo = nvcv::musa::Tensor2DWrap<int>(histogram);
    ...
    else
    {
        ...
        auto maskAccess = nvcv::musa::CreateTensorWrapNHW<uchar>(*maskTensorData);
