...
template<class T>
__global__ void rgb_to_bgr_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                int bidx)
{
    ...
    *dst.ptr(batch_idx, dst_y, dst_x, 2) = r;
    ...
    {
        T al = src.numChannels() == 4 ? *src.ptr(batch_idx, dst_y, dst_x, 3) : musa::TypeTraits<T>::max;
        *dst.ptr(batch_idx, dst_y, dst_x, 3) = al;
    }
}

template<class T>
__global__ void gray_to_bgr_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    const int batch_idx = get_batch_idx();
...
template<class T>
__global__ void bgr_to_gray_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                      int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
...
template<class T>
__global__ void bgr_to_gray_float_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                       int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
...
template<class T>
__global__ void bgr_to_yuv_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                     int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    int delta = ((T)(musa::TypeTraits<T>::max / 2 + 1)) * (1 << yuv_shift);
    ...
    *dst.ptr(batch_idx, dst_y, dst_x, 0) = musa::SaturateCast<T>(Y);
    *dst.ptr(batch_idx, dst_y, dst_x, 1) = musa::SaturateCast<T>(Cb);
    *dst.ptr(batch_idx, dst_y, dst_x, 2) = musa::SaturateCast<T>(Cr);
}

template<class T>
__global__ void bgr_to_yuv_float_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                      int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
...
template<class T>
__global__ void yuv_to_bgr_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                     int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    int delta = ((T)(musa::TypeTraits<T>::max / 2 + 1));
    ...
    *dst.ptr(batch_idx, dst_y, dst_x, bidx)     = musa::SaturateCast<T>(b);
    *dst.ptr(batch_idx, dst_y, dst_x, 1)        = musa::SaturateCast<T>(g);
    *dst.ptr(batch_idx, dst_y, dst_x, bidx ^ 2) = musa::SaturateCast<T>(r);
}

template<class T>
__global__ void yuv_to_bgr_float_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                      int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
...
template<class T>
__global__ void bgr_to_hsv_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                     int bidx, bool isFullRange)
{
    ...
    uint8_t diff = musa::SaturateCast<uint8_t>(v - vmin);
    vr           = v == r ? -1 : 0;
    vg           = v == g ? -1 : 0;

    int hdiv_table = diff == 0 ? 0 : musa::SaturateCast<int>((hrange << hsv_shift) / (6. * diff));
    int sdiv_table = v == 0 ? 0 : musa::SaturateCast<int>((255 << hsv_shift) / (1. * v));
    s              = (diff * sdiv_table + (1 << (hsv_shift - 1))) >> hsv_shift;
    h              = (vr & (g - b)) + (~vr & ((vg & (b - r + 2 * diff)) + ((~vg) & (r - g + 4 * diff))));
    h              = (h * hdiv_table + (1 << (hsv_shift - 1))) >> hsv_shift;
    h += h < 0 ? hr : 0;

    *dst.ptr(batch_idx, dst_y, dst_x, 0) = musa::SaturateCast<uint8_t>(h);
    *dst.ptr(batch_idx, dst_y, dst_x, 1) = (uint8_t)s;
    *dst.ptr(batch_idx, dst_y, dst_x, 2) = (uint8_t)v;
}

template<class T>
__global__ void bgr_to_hsv_float_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                      int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
...
template<class T>
__global__ void hsv_to_bgr_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                     int bidx, bool isFullRange)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    constexpr T     alpha   = musa::TypeTraits<T>::max;
    ...
    *dst.ptr(batch_idx, dst_y, dst_x, bidx)     = musa::SaturateCast<uchar>(b * 255.0f);
    *dst.ptr(batch_idx, dst_y, dst_x, 1)        = musa::SaturateCast<uchar>(g * 255.0f);
    *dst.ptr(batch_idx, dst_y, dst_x, bidx ^ 2) = musa::SaturateCast<uchar>(r * 255.0f);
    if (dst.numChannels() == 4)
        *dst.ptr(batch_idx, dst_y, dst_x, 3) = alpha;
}

template<class T>
__global__ void hsv_to_bgr_float_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                      int bidx)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
                                                       ...
                                                       uchar &b)
{
    ...
    int yy = musa::max(0, Y - 16) * C0;
    int uu = U - 128;
    int vv = V - 128;

    r = musa::SaturateCast<uchar>(CV_DESCALE((yy + C1 * vv), yuv4xx_shift));
    g = musa::SaturateCast<uchar>(CV_DESCALE((yy + C2 * vv + C3 * uu), yuv4xx_shift));
    b = musa::SaturateCast<uchar>(CV_DESCALE((yy + C4 * uu), yuv4xx_shift));
}
                                                       ...
                                                       uchar &U, uchar &V)
{
    ...
    Y = musa::SaturateCast<uchar>(yy >> ITUR_BT_601_SHIFT);
    ...
    U = musa::SaturateCast<uchar>(uu >> ITUR_BT_601_SHIFT);
    V = musa::SaturateCast<uchar>(vv >> ITUR_BT_601_SHIFT);
}

template<class T>
__global__ void bgr_to_yuv420sp_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src,
                                          musa::ImageBatchVarShapeWrapNHWC<T> dst, int bidx, int uidx,
...
template<class T>
__global__ void bgr_to_yuv420p_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src,
                                         musa::ImageBatchVarShapeWrapNHWC<T> dst, int bidx, int uidx,
                                         NVCVColorConversionCode code)
{
    int       src_x     = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class T>
__global__ void yuv420sp_to_bgr_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src,
                                          musa::ImageBatchVarShapeWrapNHWC<T> dst, int bidx, int uidx,
                                          NVCVColorConversionCode code)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
...
template<class T>
__global__ void yuv420p_to_bgr_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src,
                                         musa::ImageBatchVarShapeWrapNHWC<T> dst, int bidx, int uidx,
                                         NVCVColorConversionCode code)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
...
// thread.
template<class T>
__global__ void yuv422_to_bgr_char_nhwc(musa::ImageBatchVarShapeWrap<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
...
template<class T>
__global__ void yuv420_to_gray_char_nhwc(musa::ImageBatchVarShapeWrapNHWC<T> src,
                                         musa::ImageBatchVarShapeWrapNHWC<T> dst, NVCVColorConversionCode code)
{
    int       dst_x     = blockIdx.x * blockDim.x + threadIdx.x;
    int       dst_y     = blockIdx.y * blockDim.y + threadIdx.y;
    const int batch_idx = get_batch_idx();
...
template<class T>
__global__ void yuv422_to_gray_char_nhwc(musa::ImageBatchVarShapeWrap<T> src, musa::ImageBatchVarShapeWrapNHWC<T> dst,
                                         int yidx)
{
    const int batch_idx = get_batch_idx();
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_RGB(const ImageBatchVarShapeDataStridedCuda &inData,
                            ...
                            musaStream_t stream)
{
    ...
    case kCV_8S:
    {
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> src_ptr(inData, sch);
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> dst_ptr(outData, dch);
    ...
    case kCV_16S:
    {
        musa::ImageBatchVarShapeWrapNHWC<uint16_t> src_ptr(inData, sch);
        musa::ImageBatchVarShapeWrapNHWC<uint16_t> dst_ptr(outData, dch);
    ...
    case kCV_32S:
    {
        musa::ImageBatchVarShapeWrapNHWC<int32_t> src_ptr(inData, sch);
        musa::ImageBatchVarShapeWrapNHWC<int32_t> dst_ptr(outData, dch);
    ...
    case kCV_32F:
    {
        musa::ImageBatchVarShapeWrapNHWC<float> src_ptr(inData, sch);
        musa::ImageBatchVarShapeWrapNHWC<float> dst_ptr(outData, dch);
    ...
    case kCV_64F:
    {
        musa::ImageBatchVarShapeWrapNHWC<double> src_ptr(inData, sch);
        musa::ImageBatchVarShapeWrapNHWC<double> dst_ptr(outData, dch);
...
inline nvcv::legacy::musa_op::ErrorCode GRAY_to_BGR(const ImageBatchVarShapeDataStridedCuda &inData,
                             const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                             musaStream_t stream)
{
    ...
    case kCV_8S:
    {
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> dst_ptr(outData, dch);
    ...
    case kCV_16S:
    {
        musa::ImageBatchVarShapeWrapNHWC<uint16_t> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<uint16_t> dst_ptr(outData, dch);
    ...
    case kCV_32S:
    {
        musa::ImageBatchVarShapeWrapNHWC<int32_t> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<int32_t> dst_ptr(outData, dch);
    ...
    case kCV_32F:
    {
        musa::ImageBatchVarShapeWrapNHWC<float> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<float> dst_ptr(outData, dch);
    ...
    case kCV_64F:
    {
        musa::ImageBatchVarShapeWrapNHWC<double> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<double> dst_ptr(outData, dch);
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_GRAY(const ImageBatchVarShapeDataStridedCuda &inData,
                             const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                             musaStream_t stream)
{
    ...
    {
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<unsigned char> dst_ptr(outData, dcn);
    ...
    case kCV_16U:
    {
        musa::ImageBatchVarShapeWrapNHWC<unsigned short> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<unsigned short> dst_ptr(outData, dcn);
    ...
    {
        musa::ImageBatchVarShapeWrapNHWC<float> src_ptr(inData, channels);
        musa::ImageBatchVarShapeWrapNHWC<float> dst_ptr(outData, dcn);
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_YUV(const ImageBatchVarShapeDataStridedCuda &inData,
                            const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                            musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode YUV_to_BGR(const ImageBatchVarShapeDataStridedCuda &inData,
                            const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                            musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_HSV(const ImageBatchVarShapeDataStridedCuda &inData,
                            const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                            musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode HSV_to_BGR(const ImageBatchVarShapeDataStridedCuda &inData,
                            const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                            musaStream_t stream)
{
...
inline nvcv::legacy::musa_op::ErrorCode YUV420xp_to_BGR(const ImageBatchVarShapeDataStridedCuda &inData,
                                 const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                                 musaStream_t stream)
{
    int bidx
...
inline nvcv::legacy::musa_op::ErrorCode YUV422_to_BGR(const ImageBatchVarShapeDataStridedCuda &inData,
                               const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                               musaStream_t stream)
{
    int bidx
    ...
    musa::ImageBatchVarShapeWrap<uint8_t>     src_ptr(inData);
    musa::ImageBatchVarShapeWrapNHWC<uint8_t> dst_ptr(outData, dcn);
...
template<typename T>
inline static void bgr_to_yuv420p_launcher(musa::ImageBatchVarShapeWrapNHWC<T> src_ptr,
                                           musa::ImageBatchVarShapeWrapNHWC<T> dst_ptr, DataShape inputShape, int bidx,
                                           int uidx, NVCVColorConversionCode code, musaStream_t stream)
{
    // method 1
    dim3 blockSize(BLOCK, BLOCK / 4, 1);
...
template<typename T>
inline static void bgr_to_yuv420sp_launcher(musa::ImageBatchVarShapeWrapNHWC<T> src_ptr,
                                            musa::ImageBatchVarShapeWrapNHWC<T> dst_ptr, DataShape inputShape, int bidx,
                                            int uidx, NVCVColorConversionCode code, musaStream_t stream)
{
    // method 1
...
inline nvcv::legacy::musa_op::ErrorCode BGR_to_YUV420xp(const ImageBatchVarShapeDataStridedCuda &inData,
                                 const ImageBatchVarShapeDataStridedCuda &outData, NVCVColorConversionCode code,
                                 musaStream_t stream)
{
    int bidx
    ...
    musa_op::DataShape maxInputShape(inData.numImages(), channels, inData.maxSize().h, inData.maxSize().w);
