...
void runFlipKernel(SrcWrap src, DstWrap dst, Size2D dstSize, int numSamples, int32_t flipCode, musaStream_t stream)
{
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode flip(const TensorDataStridedCuda &input, const TensorDataStridedCuda &output, const int32_t flipCode,
               musaStream_t stream)
{
    ...
    {
        auto src = musa::CreateTensorWrapNHW<const T, int32_t>(input);
        auto dst = musa::CreateTensorWrapNHW<T, int32_t>(output);
                      ...
                      musaStream_t stream)
{
    ...
    musa_op::DataType dataType = GetLegacyDataType(input.dtype());
    ...
    musa_op::DataShape inputShape = GetLegacyDataShape(inputWrapper->infoShape());
    if (inputShape.C > 4)
    {
    ...
    typedef ErrorCode (*flip_t)(const TensorDataStridedCuda &input, const TensorDataStridedCuda &output,
                                const int32_t flipCode, musaStream_t stream);
