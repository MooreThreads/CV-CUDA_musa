...
{
    static nvcv::legacy::musa_op::ErrorCode call(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                          Transform transform, const float4 &borderValue, musaStream_t stream)
    {
        ...
        {
            auto src = musa::CreateInterpolationWrapNHW<const T, B, I, int32_t>(inData, bVal);
...
nvcv::legacy::musa_op::ErrorCode warp_caller(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData, Transform transform,
                      int interpolation, int borderMode, const float4 &borderValue, musaStream_t stream)
{
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode warpAffine(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
...
template<typename T>
nvcv::legacy::musa_op::ErrorCode warpPerspective(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                                 ...
                                 const float4 borderValue, musaStream_t stream)
{
    ...
    if (!(flags & NVCV_WARP_INVERSE_MAP))
    {
        musa::math::Matrix<float, 3, 3> tempMatrixForInverse;

        tempMatrixForInverse.load(transMatrix);

        musa::math::inv_inplace(tempMatrixForInverse);
