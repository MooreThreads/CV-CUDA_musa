...
void calculateRotateCoefCUDA(musa::Tensor2DWrap<float> rotateCoefBuf, const int degrees, const musaStream_t &stream);

template<typename T>
__global__ void resetRotatedPointsBuf(musa::Tensor3DWrap<T> rotatedPointsTensor, const int numOfDegrees)
{
...
__global__ void calculateRotateCoef(musa::Tensor2DWrap<float> aCoeffs, const int degrees)
{
...
template<typename T>
__global__ void calculateRotateArea(musa::Tensor3DWrap<T>   inContourPointsData,
                                    musa::Tensor3DWrap<int> rotatedPointsTensor, musa::Tensor2DWrap<float> rotateCoeffs,
                                    musa::Tensor2DWrap<int> numPointsInContourBuf)
{
...
__global__ void findMinAreaAndAngle(TensorWrapper rotatedPointsTensor, musa::Tensor2DWrap<float> outMinAreaRectBox,
...
void minAreaRect(const TensorDataStridedCuda &inData, void *rotatedPointsDev,
                 const musa::Tensor2DWrap<float> rotateCoeffsData, const TensorDataStridedCuda &numPointsInContour,
                 const TensorDataStridedCuda &outData, int contourBatch, int maxNumPointsInContour, musaStream_t stream)
{
    auto inAccess = nvcv::TensorDataAccessStrided::Create(inData);
    NVCV_ASSERT(inAccess);

    musa::Tensor3DWrap<T> inContourPointsData(inData);
    ...
    musa::Tensor3DWrap<int>   rotatedPointsTensor(rotatedPointsDev, kernelPitch1, kernelPitch2);
    musa::Tensor2DWrap<float> outMinAreaRectData(outData);
    musa::Tensor2DWrap<int>   pointsInContourData(numPointsInContour);
    ...
    : mMaxContourNum(maxContourNum)
{
    ...
    NVCV_CHECK_THROW(musaMalloc(&mRotateCoeffsBufDev, _MAX_ROTATE_DEGREES * 2 * sizeof(float)));
    NVCV_CHECK_THROW(musaMalloc(&mRotatedPointsDev, rotatedPtsAllocSize));
}

MinAreaRect::~MinAreaRect()
{
    NVCV_CHECK_LOG(musaFree(mRotateCoeffsBufDev));
    NVCV_CHECK_LOG(musaFree(mRotatedPointsDev));
}
...
ErrorCode MinAreaRect::infer(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &outData,
                             ...
                             musaStream_t stream)
{
    musa_op::DataType input_datatype  = GetLegacyDataType(inData.dtype());
    musa_op::DataType output_datatype = GetLegacyDataType(outData.dtype());
...
#    pragma GCC diagnostic pop
    ...
    musa::Tensor2DWrap<float> rotateCoeffsData(mRotateCoeffsBufDev, static_cast<int>(2 * sizeof(float)));
    ...
    typedef void (*minAreaRect_t)(const TensorDataStridedCuda &inData, void *rotatedPointsDev,
                                  const musa::Tensor2DWrap<float> rotateCoeffsData,
                                  const TensorDataStridedCuda &numPointsInContour, const TensorDataStridedCuda &outData,
                                  int batch, int maxNumPointsInContour, musaStream_t stream);
