                                           ...
                                           const int columns)
{
    ...
    for (int c = colIdx - radius; c < colIdx + radius + 2; c++)
    {
        for (int r = rowIdx - radius; r < rowIdx + radius + 2; r++)
        {
            ...
            work_type curr      = musa::StaticCast<float>(src[coord]);
            work_type currColor = musa::StaticCast<float>(srcColor[coord]);
    ...
    {
        dst[coord0] = nvcv::musa::SaturateCast<T>(numerator0 / denominator0);
    }
    if (colIdx + 1 < columns && rowIdx < rows)
    {
        dst[coord1] = nvcv::musa::SaturateCast<T>(numerator1 / denominator1);
    }
    if (colIdx < columns && rowIdx + 1 < rows)
    {
        dst[coord2] = nvcv::musa::SaturateCast<T>(numerator2 / denominator2);
    }
    if (colIdx + 1 < columns && rowIdx + 1 < rows)
    {
        dst[coord3] = nvcv::musa::SaturateCast<T>(numerator3 / denominator3);
    }
}

template<typename T, NVCVBorderType B>
nvcv::legacy::musa_op::ErrorCode JointBilateralFilterCaller(const TensorDataStridedCuda &inData, const TensorDataStridedCuda &inColorData,
                                     ...
                                     musaStream_t stream)
{
    ...
    {
        auto src      = musa::CreateBorderWrapNHW<const T, B, int32_t>(inData, musa::SetAll<T>(borderValue));
        auto srcColor = musa::CreateBorderWrapNHW<const T, B, int32_t>(inColorData, musa::SetAll<T>(borderValue));
        auto dst      = musa::CreateTensorWrapNHW<T, int32_t>(outData);
    ...
    {
        LOG_ERROR("Input size exceeds " << musa::TypeTraits<int32_t>::max << ". Tensor is too large.");
                                      ...
                                      NVCVBorderType borderMode, musaStream_t stream)
{
    ...
    musa_op::DataFormat inputColor_format = GetLegacyDataFormat(inColorData.layout());
    ...
    typedef ErrorCode (*joint_bilateral_filter_t)(
        ...
        float sigmaSpace, float borderValue, musaStream_t stream);
