...
public:
    ...
    void operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out, const int top, const int left,
                    const NVCVBorderType borderMode, const float4 borderValue) const;
    void operator()(musaStream_t stream, const nvcv::ImageBatch &in, const nvcv::ImageBatch &out,
                    const nvcv::Tensor &top, const nvcv::Tensor &left, const NVCVBorderType borderMode,
                    const float4 borderValue) const;
    void operator()(musaStream_t stream, const nvcv::ImageBatch &in, const nvcv::Tensor &out, const nvcv::Tensor &top,
                    const nvcv::Tensor &left, const NVCVBorderType borderMode, const float4 borderValue) const;

private:
    std::unique_ptr<nvcv::legacy::musa_op::CopyMakeBorder>         m_legacyOp;
    std::unique_ptr<nvcv::legacy::musa_op::CopyMakeBorderVarShape> m_legacyOpVarShape;
};
