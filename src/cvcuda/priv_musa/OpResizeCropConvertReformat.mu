...
public:
    ...
    template <typename SrcT, class = musa::Require<musa::HasTypeTraits<SrcT> > >
    __device__ __forceinline__
    void operator()(const uint n, const int y, const int x, const SrcT val)
    {
        static_assert(musa::NumElements<SrcT> == N);

        // Set destination pointer to correct pixel (batch, row, & column).
        DstT *dst = ptr(n, y, x);

        // Shuffle pixel channels.
        if constexpr (musa::NumComponents<SrcT> > 1) {
            dst[m_mapC[0]] = musa::SaturateCast<DstT>(val.x);
            dst[m_mapC[1]] = musa::SaturateCast<DstT>(val.y);
            if constexpr (N >= 3) dst[m_mapC[2]] = musa::SaturateCast<DstT>(val.z);
            if constexpr (N == 4) dst[m_mapC[3]] = musa::SaturateCast<DstT>(val.w);
        }
        else if constexpr (musa::NumComponents<SrcT> == 1)
             *dst = musa::SaturateCast<DstT>(val.x);
        else *dst = musa::SaturateCast<DstT>(val);
    }
                                    ...
                                    const float scale, const float offset, bool src_cast)
{
    ...
    if (dst_x < dst.width() && dst_y < dst.height())
    {
        ...
        int sx1 = musa::min(sx0 + 1, src_w - 1);
        int sy1 = musa::min(sy0 + 1, src_h - 1);
        ...
        if (src_cast)
            dst(blockIdx.z, dst_y, dst_x,
                scale * musa::SaturateCast<SrcT>((1-fy) * ((1-fx) * ptr0[0] + ptr0[sx1] * fx)
...
{
    constexpr uint NumElems = musa::NumElements<SrcT>;

    using SrcBaseT = musa::BaseType<SrcT>;
    using DstBaseT = musa::BaseType<DstT>;
    ...
    auto src = musa::CreateTensorWrapNHW<const SrcT, StrideT>(srcData);
...
void resizeCropConvertReformat(const nvcv::ImageBatchVarShapeDataStridedCuda &srcData,
                               ...
                               float scale, float offset, bool srcCast, musaStream_t stream)
{
    constexpr uint NumElems = musa::NumElements<SrcT>;
    ...
    musa::ImageBatchVarShapeWrap<const SrcT> src(srcData);
...
void ResizeCropConvertReformat::operator()(musaStream_t stream, const nvcv::Tensor &src, const nvcv::Tensor &dst,
                                           ...
                                           bool srcCast) const
{
    ...
    if (!srcData)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input must be a musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Output must be a musa-accessible, pitch-linear tensor");
    }
    ...
    musa_op::DataType srcType = helpers::GetLegacyDataType((*srcData).dtype());
    musa_op::DataType dstType = helpers::GetLegacyDataType((*dstData).dtype());

    if (srcType != musa_op::kCV_8U)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_NOT_COMPATIBLE, "Input must be of data type uchar.");
    }

    if (dstType != musa_op::kCV_8U && dstType != musa_op::kCV_32F)
    {
    ...
    if (srcAccess->sampleStride() * samples > musa::TypeTraits<int32_t>::max)
    {
...
void ResizeCropConvertReformat::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &src,
                                           ...
                                           const NVCVChannelManip manip, float scale, float offset, bool srcCast) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input data must be a musa-accessible, varshape image batch");
    }
    ...
    musa_op::DataType srcType = helpers::GetLegacyDataType(srcFrmt);
    musa_op::DataType dstType = helpers::GetLegacyDataType((*dstData).dtype());
