...
#include "legacy/CvCudaUtils.muh"
...
namespace legacy = nvcv::legacy::musa_op;
                                     ...
                                     const RGB2YUVConstants cooef)
{
    ...
    *dst.ptr(batch_idx, dst_y, dst_x, 1) = musa::SaturateCast<T>(U);
    *dst.ptr(batch_idx, dst_y, dst_x, 2) = musa::SaturateCast<T>(V);
}
                                                  ...
                                                  const YUV2RGBConstants cooef)
{
    ...
    b = musa::SaturateCast<T>(B);
    g = musa::SaturateCast<T>(G);
    r = musa::SaturateCast<T>(R);
}
                                          ...
                                          const RGB2YUVConstants cooef)
{
    ...
    // Write the Y plane
    *dst.ptr(batch_idx, src_y, src_x, 0) = musa::SaturateCast<T>(Y0);
    ...
    int delta = ((T)(musa::TypeTraits<T>::max / 2 + 1)); // non scaled delta in this kernel

    *dst.ptr(batch_idx, srcSize.y + src_y / 2, uv_x + uidx) = musa::SaturateCast<T>((U0 + U1 + U2 + U3) / 4 + delta);
    *dst.ptr(batch_idx, srcSize.y + src_y / 2, uv_x + (1 - uidx))
        = musa::SaturateCast<T>((V0 + V1 + V2 + V3) / 4 + delta);
}
...
void AdvCvtColor::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
...
void AdvCvtColor::Yuv2Bgr(musaStream_t stream, const nvcv::TensorDataStridedCuda &in,
                          ...
                          nvcv::ColorSpec spec) const
{
    ...
    nvcv::legacy::musa_op::DataType  inDataType  = nvcv::legacy::helpers::GetLegacyDataType(inAccess->dtype());
    nvcv::legacy::musa_op::DataShape inputShape  = nvcv::legacy::helpers::GetLegacyDataShape(inAccess->infoShape());
    nvcv::legacy::musa_op::DataShape outputShape = nvcv::legacy::helpers::GetLegacyDataShape(outAccess->infoShape());
    ...
    case legacy::kCV_8U:
    {
        ...
        if (std::max(outMaxStride, inMaxStride) <= musa::TypeTraits<int32_t>::max)
        {
            auto srcWrap = musa::CreateTensorWrapNHWC<uint8_t, int32_t>(in);
            auto dstWrap = musa::CreateTensorWrapNHWC<uint8_t, int32_t>(out);
...
void AdvCvtColor::Bgr2Yuv(musaStream_t stream, const nvcv::TensorDataStridedCuda &in,
                          const nvcv::TensorDataStridedCuda &out, NVCVColorConversionCode code,
                          nvcv::ColorSpec spec) const
{
...
void AdvCvtColor::NvYuv2Bgr(musaStream_t stream, const nvcv::TensorDataStridedCuda &in,
                            const nvcv::TensorDataStridedCuda &out, NVCVColorConversionCode code,
                            nvcv::ColorSpec spec) const
{
    // blue index
...
void AdvCvtColor::Bgr2NvYuv(musaStream_t stream, const nvcv::TensorDataStridedCuda &in,
                            const nvcv::TensorDataStridedCuda &out, NVCVColorConversionCode code,
                            nvcv::ColorSpec spec) const
{
    // blue index
