...
void GaussianNoise::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
                               ...
                               unsigned long long seed) const
{
    ...
    if (muData == nullptr)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "mu must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "sigma must be musa-accessible, pitch-linear tensor");
    }

    NVCV_CHECK_THROW(m_legacyOp->infer(*inData, *outData, *muData, *sigmaData, per_channel, seed, stream));
}

void GaussianNoise::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in,
