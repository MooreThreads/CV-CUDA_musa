...
inline static bool check_error(musaError_t e, const char *call, int line, const char *file)
{
    if (e != musaSuccess)
    {
        fprintf(stderr, "CUDA Runtime error %s # %s, code = %s [ %d ] in file %s:%d\n", call, musaGetErrorString(e),
                musaGetErrorName(e), e, file, line);
        return false;
    }
    return true;
}
...
public:
    ...
    NVCVSegment(NVCVBoxI _box, int32_t _thickness, float *_hSeg, int32_t _segWidth, int32_t _segHeight,
        ...
        , segColor(_segColor)
    {
        checkERR(musaMalloc(&dSeg, static_cast<size_t>(segWidth) * segHeight * sizeof(float)));
        checkERR(
            musaMemcpy(dSeg, _hSeg, static_cast<size_t>(segWidth) * segHeight * sizeof(float), musaMemcpyHostToDevice));
    }
    ...
    {
        checkERR(musaMalloc(&dSeg, static_cast<size_t>(segWidth) * segHeight * sizeof(float)));
        checkERR(musaMemcpy(dSeg, segment.dSeg, static_cast<size_t>(segWidth) * segHeight * sizeof(float),
...
public:
        ...
        , interpolation(_interpolation)
    {
        ...
        checkERR(musaMalloc(&dPoints, 2 * numPoints * sizeof(int)));

        std::copy_n(_hPoints, 2 * numPoints, hPoints);
        checkERR(musaMemcpy(dPoints, _hPoints, 2 * numPoints * sizeof(int), musaMemcpyHostToDevice));
    }
        ...
        , interpolation(pl.interpolation)
    {
        ...
        checkERR(musaMemcpy(dPoints, pl.dPoints, 2 * numPoints * sizeof(int), musaMemcpyDeviceToDevice));
    }
