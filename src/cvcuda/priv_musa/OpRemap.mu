...
#include <cvcuda/cuda_tools_musa/InterpolationVarShapeWrap.hpp>
                                                          ...
                                                          bool alignCorners, NVCVRemapMapValueType mapValueType)
{
    ...
    case NVCV_REMAP_ABSOLUTE:
        ...
        params.mapScale  = musa::StaticCast<float>(mapSize) / dstSize;
    ...
    case NVCV_REMAP_RELATIVE_NORMALIZED:
        params.srcScale  = musa::StaticCast<float>(srcSize) / dstSize;
...
void RunRemap(musaStream_t stream, const DataStridedCuda &srcData, const DataStridedCuda &dstData,
              ...
              int2 mapSize, int mapNumSamples)
{
    ...
    if constexpr (std::is_same_v<DataStridedCuda, nvcv::TensorDataStridedCuda>)
    {
        ...
        int2 srcSize = musa::StaticCast<int>(long2{srcAccess->numCols(), srcAccess->numRows()});
        int2 dstSize = musa::StaticCast<int>(long2{dstAccess->numCols(), dstAccess->numRows()});
        ...
        if (std::max(srcMaxStride, dstMaxStride) <= musa::TypeTraits<int32_t>::max)
        {
            auto src = musa::CreateInterpolationWrapNHW<const T, B, SI, int32_t>(srcData, borderValue);
            auto dst = musa::CreateTensorWrapNHW<T, int32_t>(dstData);
    ...
    else
    {
        ...
        musa::InterpolationVarShapeWrap<const T, B, SI> src(srcData, borderValue);
        musa::ImageBatchVarShapeWrap<T>                 dst(dstData);
              ...
              const T &borderValue)
{
    ...
    int2 mapSize       = musa::StaticCast<int>(long2{mapAccess->numCols(), mapAccess->numRows()});
    int  mapNumSamples = mapAccess->numSamples();

    if (mapAccess->sampleStride() * mapAccess->numSamples() <= musa::TypeTraits<int32_t>::max)
    {
        auto map = musa::CreateInterpolationWrapNHW<const float2, kMapBorderType, MI, int32_t>(mapData);
...
{
    const T bvalue = musa::DropCast<musa::NumElements<T>>(musa::StaticCast<musa::BaseType<T>>(borderValue));
...
inline void RunRemap(musaStream_t stream, const DataStridedCuda &srcData, const DataStridedCuda &dstData,
                     const nvcv::TensorDataStridedCuda &mapData, NVCVInterpolationType srcInterp,
                     NVCVInterpolationType mapInterp, NVCVRemapMapValueType mapValueType, bool alignCorners,
...
#define NVCV_RUN_REMAP(BT, DT, T)                                                          \
    ((dataType == nvcv::TYPE_##BT && numChannels == musa::NumElements<T>) ||               \
...
void Remap::operator()(musaStream_t stream, const nvcv::Tensor &src, const nvcv::Tensor &dst, const nvcv::Tensor &map,
                       ...
                       float4 borderValue) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Remap map input must be musa-accessible, pitch-linear tensor");
    }
...
void Remap::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &src, const nvcv::ImageBatchVarShape &dst,
