...
using ArgWrapper = musa::Tensor1DWrap<const ArgT, int32_t>;
                                            ...
                                            const int p)
{
    ...
    using IntermediateT              = decltype(ArgT{} * SrcT{});
    using SrcBT                      = musa::BaseType<SrcT>;
    using DstBT                      = musa::BaseType<DstT>;
    using BI                         = musa::BaseType<IntermediateT>;
    static constexpr int numChannels = musa::NumElements<SrcT>;
    static_assert(numChannels == musa::NumElements<DstT>);
    ...
    int3 nhwCoord = {static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x),
                                   ...
                                   int numPlanes)
{
    ...
    using SrcBT    = musa::BaseType<typename SrcWrapper::ValueType>;
    int  z         = blockIdx.z;
...
inline void RunBrightnessContrast(musaStream_t stream, const SrcData &srcData, const DstData &dstData,
                                  const BatchArgsWrap<ArgT> &batchArgs)
{
    ...
    if constexpr (std::is_same_v<SrcData, nvcv::TensorDataStridedCuda>)
    {
        ...
        int2 size      = musa::StaticCast<int>(long2{srcAccess->numCols(), srcAccess->numRows()});
        ...
        if constexpr (!isPlanar)
        {
            auto src = musa::CreateTensorWrapNHW<const SrcValueT, StrideType>(srcData);
            auto dst = musa::CreateTensorWrapNHW<DstValueT, StrideType>(dstData);
            BrightnessContrast<isPlanar><<<grid, block, 0, stream>>>(src, dst, batchArgs, size, 1);
        }
        else
        {
            auto src = musa::Tensor4DWrap<const SrcValueT, StrideType>(
            ...
            auto dst = musa::Tensor4DWrap<DstValueT, StrideType>(
    ...
    else
    {
        ...
        musa::ImageBatchVarShapeWrap<const SrcValueT> src(srcData);
        musa::ImageBatchVarShapeWrap<DstValueT>       dst(dstData);
...
#define NVCV_BRIGHTNESS_RUN_CHANNELS(NUM_CHANNELS)                             \
    ...
    {                                                                          \
        using SrcVal = musa::MakeType<SrcValBase, NUM_CHANNELS>;               \
        using DstVal = musa::MakeType<DstValBase, NUM_CHANNELS>;               \
        ...
        [&](auto dummySrcVal)
        {
            ...
            RunTypeSwitch(
                ...
                [&](auto dummyDstVal)
                {
                    ...
                    if (numChannels == 1)
                    {
                        using SrcVal = musa::MakeType<SrcValBase, 1>;
                        using DstVal = musa::MakeType<DstValBase, 1>;
                        if (numPlanes == 1)
                        {
                            cb(SrcVal{}, DstVal{}, ArgT{}, std::integral_constant<bool, false>{});
                        }
...
inline auto validateSrcDstVarBatch(int &numSamples, int &numInterleavedChannels, int &numPlanes,
                                   nvcv::DataType &srcDtype, nvcv::DataType &dstDtype, musaStream_t stream,
                               ...
                               const nvcv::Tensor &brightnessShift, const nvcv::Tensor &contrastCenter)
{
                               ...
                               const nvcv::Tensor &argTensor)
    {
        if (argTensor)
        {
            ...
            {
                throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                      "The %s argument must be musa-accessible, pitch-linear tensor", argName.c_str());
            }
            if (argData->rank() > 1)
            {
                throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
...
void BrightnessContrast::operator()(musaStream_t stream, const nvcv::Tensor &src, const nvcv::Tensor &dst,
                                    const nvcv::Tensor &brightness, const nvcv::Tensor &contrast,
...
void BrightnessContrast::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &src,
