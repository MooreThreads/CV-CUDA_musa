...
void CopyMakeBorder::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out, const int top,
...
void CopyMakeBorder::operator()(musaStream_t stream, const nvcv::ImageBatch &in, const nvcv::Tensor &out,
                                ...
                                const float4 borderValue) const
{
    ...
    if (topData == nullptr)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "Top must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Left must be musa-accessible, pitch-linear tensor");
    }
...
void CopyMakeBorder::operator()(musaStream_t stream, const nvcv::ImageBatch &in, const nvcv::ImageBatch &out,
                                const nvcv::Tensor &top, const nvcv::Tensor &left, const NVCVBorderType borderMode,
                                const float4 borderValue) const
{
    auto inData = in.exportData<nvcv::ImageBatchVarShapeDataStridedCuda>(stream);
