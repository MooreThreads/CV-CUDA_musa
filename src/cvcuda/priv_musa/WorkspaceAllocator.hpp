...
public:
     ...
     */
    WorkspaceMemAllocator(const WorkspaceMem &mem, std::optional<musaStream_t> acquireReleaseStream = std::nullopt)
     ...
     */
    WorkspaceMemAllocator(const WorkspaceMem &mem, std::optional<musaStream_t> acquireStream,
                          std::optional<musaStream_t> releaseStream)
        : m_mem(mem)
     ...
     */
    void acquire(std::optional<musaStream_t> stream)
    {
        if (m_acquired)
        ...
        if (m_mem.ready)
        {
            if (stream)
            {
                if (musaStreamWaitEvent(*stream, m_mem.ready) != musaSuccess)
                    throw nvcv::Exception(nvcv::Status::ERROR_INTERNAL, "musaStreamWairEvent failed");
            }
            else
            {
                if (musaEventSynchronize(m_mem.ready) != musaSuccess)
                    throw nvcv::Exception(nvcv::Status::ERROR_INTERNAL, "musaEventSynchronize failed");
            }
        }
        m_acquired = true;
    }
     ...
     */
    void release(std::optional<musaStream_t> stream)
    {
        if (m_released)
        ...
        if (m_mem.ready && m_offset)
        {
            ...
            if (stream)
                if (musaEventRecord(m_mem.ready, *stream) != musaSuccess)
                    throw nvcv::Exception(nvcv::Status::ERROR_INTERNAL, "musaEventRecord failed");
        }
        m_released = true;
    }
...
private:
    ...
    std::optional<musaStream_t> m_acquireStream, m_releaseStream;
};
...
public:
    ...
    {
        return musaMem.get<T>(count, alignment);
    }

    WorkspaceMemAllocator hostMem;
    WorkspaceMemAllocator pinnedMem;
    WorkspaceMemAllocator musaMem;
};
