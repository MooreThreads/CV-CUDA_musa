...
#include <cvcuda/cuda_tools_musa/BorderVarShapeWrap.hpp>
...
__device__ void transfer_data(musa::BorderVarShapeWrap<const T1, B> srcWrap, DstWrapper dstWrap, int2 src_idx,
                              ...
                              int scale_channels, bool dst_planar)
{
    if (dst_planar)
    {
        for (int c = 0; c < ch; c++)
        {
            ...
            dstWrap[(int4){dst_idx.x, dst_idx.y, c, batchidx}] = musa::SaturateCast<typename DstWrapper::ValueType>(
    ...
    else
    {
        for (int c = 0; c < ch; c++)
        {
            float base  = get_base_value(baseWrap, c, base_channels);
            float scale = get_scale_value(scaleWrap, c, scale_channels, epsilon, flags);
            dstWrap[(int4){c, dst_idx.x, dst_idx.y, batchidx}] = musa::SaturateCast<typename DstWrapper::ValueType>(
                (srcWrap[(int4){src_idx.x, src_idx.y, c, batchidx}] - base) * scale * global_scale + global_shift);
        }
    }
}
...
template<class T1, NVCVBorderType B, class DstWrapper, class TensorWrapper>
__device__ void transfer_data(musa::BorderVarShapeWrapNHWC<const T1, B> srcWrap, DstWrapper dstWrap, int2 src_idx,
                              int2 dst_idx, int batchidx, int ch, TensorWrapper baseWrap, TensorWrapper scaleWrap,
                              float global_scale, float global_shift, float epsilon, uint32_t flags, int base_channels,
                              int scale_channels, bool dst_planar)
{
...
__device__ void set_data(DstWrapper dstWrap, int2 dst_idx, int batchidx, int ch, T val, bool dst_planar)
{
    if (dst_planar)
    {
        for (int c = 0; c < ch; c++)
        {
            dstWrap[(int4){dst_idx.x, dst_idx.y, c, batchidx}] = musa::StaticCast<typename DstWrapper::ValueType>(val);
        }
    }
    else
    {
        for (int c = 0; c < ch; c++)
        {
            dstWrap[(int4){c, dst_idx.x, dst_idx.y, batchidx}] = musa::StaticCast<typename DstWrapper::ValueType>(val);
        }
    }
}
...
{
    int3 dstCoord = {static_cast<int>(blockIdx.x * blockDim.x + threadIdx.x),
...
void RunCropFlipNormalizeReformatS(musaStream_t stream, const nvcv::ImageBatchVarShapeDataStridedCuda &srcData,
                                   ...
                                   float epsilon, uint32_t flags, int channel, int3 out_size, dim3 block, dim3 grid)
{
    ...
    auto                             cropRectWrap = musa::CreateTensorWrapNHWC<int, int32_t>(cropRect);
    musa::Tensor1DWrap<int, int32_t> flipCodeWrap(flipCodeData);
    auto                             baseWrap  = musa::CreateTensorWrapNHWC<float, int32_t>(baseData);
    auto                             scaleWrap = musa::CreateTensorWrapNHWC<float, int32_t>(scaleData);

    if (src_planar && dst_planar)
    {
        musa::ImageBatchVarShapeWrap<const T_Src> srcWrap(srcData); // planar
        musa::BorderVarShapeWrap<const T_Src, B>  srcBorderWrap(srcWrap, static_cast<T_Src>(borderValue));
        musa::Tensor4DWrap<T_Dst, StrideType>     dstWrap(dstData); // planar
    ...
    else if (src_planar)
    {
        musa::ImageBatchVarShapeWrap<const T_Src> srcWrap(srcData); // planar
        musa::BorderVarShapeWrap<const T_Src, B>  srcBorderWrap(srcWrap, static_cast<T_Src>(borderValue));
        auto dstWrap = musa::CreateTensorWrapNHWC<T_Dst, StrideType>(dstData); // interleaved
        slice_flip_normalize<<<grid, block, 0, stream>>>(srcBorderWrap, dstWrap, flipCodeWrap, baseWrap, scaleWrap,
                                                         cropRectWrap, global_scale, shift, epsilon, flags, channel,
                                                         base_channels, scale_channels, out_size, dst_planar);
    }
    ...
    {
        musa::ImageBatchVarShapeWrapNHWC<const T_Src> srcWrap(srcData, channel); // interleaved
        musa::BorderVarShapeWrapNHWC<const T_Src, B>  srcBorderWrap(srcWrap, static_cast<T_Src>(borderValue));
        musa::Tensor4DWrap<T_Dst, StrideType>         dstWrap(dstData); // planar
        slice_flip_normalize<<<grid, block, 0, stream>>>(srcBorderWrap, dstWrap, flipCodeWrap, baseWrap, scaleWrap,
                                                         cropRectWrap, global_scale, shift, epsilon, flags, channel,
                                                         base_channels, scale_channels, out_size, dst_planar);
...
void RunCropFlipNormalizeReformat(musaStream_t stream, const nvcv::ImageBatchVarShapeDataStridedCuda &srcData,
                                  const nvcv::TensorDataStridedCuda &dstData,
                                  const nvcv::TensorDataStridedCuda &flipCodeData,
                                  const nvcv::TensorDataStridedCuda &baseData,
                                  const nvcv::TensorDataStridedCuda &scaleData, const float borderValue,
                                  ...
                                  float epsilon, uint32_t flags, int channel)
{
    ...
    if (outMaxStride <= musa::TypeTraits<int32_t>::max)
    {
...
{
    typedef void (*func_t)(musaStream_t stream, const nvcv::ImageBatchVarShapeDataStridedCuda &srcData,
...
void CropFlipNormalizeReformat::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in,
                                           ...
                                           uint32_t flags) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input flipCode must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input base must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input scale must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "Input cropRect must be musa-accessible, pitch-linear tensor");
    }
