...
constexpr int kIntMax = musa::TypeTraits<int>::max; // maximum number represented in an int
...
public:
    ...
    inline __device__ void load(musa::Tensor3DWrap<T> set, int sampleIdx, int setIdx, int numDim)
    {
...
template<typename T, class = musa::Require<std::is_floating_point_v<T>>>
...
template<typename T, class = musa::Require<std::is_integral_v<T>>>
...
{
    sortedDist = musa::TypeTraits<float>::max;
    sortedIdx  = -1;
...
inline __device__ void WriteMatch(int matchIdx, int set1Idx, int set2Idx, int sampleIdx, float &distance,
                                  musa::Tensor3DWrap<int> matches, musa::Tensor2DWrap<float> distances)
{
    ...
    *matches.ptr(sampleIdx, matchIdx, 1) = set2Idx;
    ...
    if (distances.ptr(0) != nullptr)
    {
        ...
        {
            distance = musa::sqrt(distance); // square-root was postpone for writing time, which is now
        }
...
__global__ void BruteForceMatcher(musa::Tensor3DWrap<ST> set1, musa::Tensor3DWrap<ST> set2,
                                  musa::Tensor1DWrap<const int> numSet1, musa::Tensor1DWrap<const int> numSet2,
                                  musa::Tensor3DWrap<int> matches, musa::Tensor1DWrap<int> numMatches,
                                  musa::Tensor2DWrap<float> distances, int set1Capacity, int set2Capacity,
...
__global__ void WriteNumMatches(musa::Tensor1DWrap<const int> numSet1, musa::Tensor1DWrap<int> numMatches,
...
inline void RunBruteForceMatcherForNorm(musaStream_t stream, const nvcv::Tensor &set1, const nvcv::Tensor &set2,
...
{
    musa::Tensor3DWrap<const SrcT>    w_set1, w_set2; // tensor wraps of set1 and set2 and other tensors
    musa::Tensor1DWrap<const int32_t> w_numSet1, w_numSet2;
    musa::Tensor3DWrap<int32_t>       w_matches;
    musa::Tensor1DWrap<int32_t>       w_numMatches;
    musa::Tensor2DWrap<float>         w_distances;
...
#undef CVCUDA_BFM_WRAP
    ...
    if (crossCheck)
    {
        ...
        NVCV_CHECK_THROW(musaMemsetAsync(w_numMatches.ptr(0), 0, sizeof(int32_t) * numSamples, stream));
    }
    else
    {
...
inline void RunBruteForceMatcherForType(musaStream_t stream, const nvcv::Tensor &set1, const nvcv::Tensor &set2,
                                        const nvcv::Tensor &numSet1, const nvcv::Tensor &numSet2,
                                        const nvcv::Tensor &matches, const nvcv::Tensor &numMatches,
                                        const nvcv::Tensor &distances, bool crossCheck, int matchesPerPoint,
...
inline void RunBruteForceMatcher(musaStream_t stream, const nvcv::Tensor &set1, const nvcv::Tensor &set2,
...
void PairwiseMatcher::operator()(musaStream_t stream, const nvcv::Tensor &set1, const nvcv::Tensor &set2,
                                 const nvcv::Tensor &numSet1, const nvcv::Tensor &numSet2, const nvcv::Tensor &matches,
                                 const nvcv::Tensor &numMatches, const nvcv::Tensor &distances, bool crossCheck,
                                 int matchesPerPoint, NVCVNormType normType)
{
