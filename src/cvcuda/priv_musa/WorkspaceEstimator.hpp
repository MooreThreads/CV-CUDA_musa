...
struct WorkspaceEstimator
{
    ...
    WorkspaceMemEstimator musaMem{0, kDefaultDeviceAlignment};

    template<typename T = char>
    WorkspaceEstimator &add(bool host, bool pinned, bool musa, size_t count = 1, size_t alignment = alignof(T))
    {
        if (host)
