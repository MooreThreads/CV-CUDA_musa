...
void AdaptiveThreshold::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
...
void AdaptiveThreshold::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in,
                                   ...
                                   const nvcv::Tensor &c) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "maxValue must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "blockSize must be musa-accessible, pitch-linear tensor");
    }

    auto cData = c.exportData<nvcv::TensorDataStridedCuda>();
    if (cData == nullptr)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "C must be musa-accessible, pitch-linear tensor");
    }
