...
void Threshold::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
                           const nvcv::Tensor &thresh, const nvcv::Tensor &maxval) const
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "thresh must be musa-accessible, pitch-linear tensor");
    }
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "maxval must be musa-accessible, pitch-linear tensor");
    }

    NVCV_CHECK_THROW(m_legacyOp->infer(*inData, *outData, *threshData, *maxvalData, stream));
}

void Threshold::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in, const nvcv::ImageBatchVarShape &out,
                           const nvcv::Tensor &thresh, const nvcv::Tensor &maxval) const
{
