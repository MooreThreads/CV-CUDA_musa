...
#include "OpHQResizeBatchWrap.muh"
#include "OpHQResizeFilter.muh"

#include <musa_runtime.h>
#include <cvcuda/cuda_tools_musa/DropCast.hpp>
#include <cvcuda/cuda_tools_musa/ImageBatchVarShapeWrap.hpp>
...
#include <cvcuda/cuda_tools_musa/StaticCast.hpp>
#include <cvcuda/cuda_tools_musa/TensorWrap.hpp>
...
#include <nvcv/util_musa/Assert.h>
#include <nvcv/util_musa/CheckError.hpp>
#include <nvcv/util_musa/Math.hpp>
...
using Vec = typename musa::MakeType<T, N>;
...
template<typename T, class = musa::Require<musa::IsCompound<T>>>
inline std::enable_if_t<std::is_integral_v<musa::BaseType<T>>, int64_t> Volume(const T &v)
{
    int64_t vol = 1;
    for (int i = 0; i < musa::NumComponents<T>; i++)
    {
        vol *= musa::GetElement(v, i);
    }
    return vol;
}
    ...
    LoadPixelLdg(const Wrap wrap, const NumChannelsT numChannels, const Idxs... idxs)
{
    ...
    using BT                      = musa::BaseType<T>;
    constexpr int kStaticChannels = NumChannelsT::kStaticChannels;
    static_assert(kStaticChannels == musa::NumElements<T>);
...
#pragma unroll
        ...
        {
            musa::GetElement(value, c) = __ldg(basePtr + c);
        }
        return value;
    }
}
...
std::enable_if_t<!NumChannelsT::kHasStaticChannels, typename Wrap::ValueType> __forceinline__ __device__
    LoadPixelLdg(const Wrap wrap, const NumChannelsT numChannels, const Idxs... idxs)
{
    static_assert(!musa::IsCompound<typename Wrap::ValueType>);
    return __ldg(GetWrapPtr(wrap, idxs...));
}
                                         ...
                                         const VecI<kSpatialNDim> inShape, const NumChannelsT numChannels)
{
    ...
    static_assert(!NumChannelsT::kHasStaticChannels || NumChannelsT::kStaticChannels == musa::NumElements<InT>);
    static_assert(musa::NumElements<OutT> == musa::NumElements<InT>);
    ...
    ForAllPixels(lo, hi,
                 ...
                 {
                     VecI<kSpatialNDim> inIdxs = musa::round<musa::RoundMode::DOWN, int>(outIdxs * scale + origin);
                     inIdxs                    = musa::clamp(inIdxs, musa::SetAll<VecI<kSpatialNDim>>(0), inShape - 1);
                                       ...
                                       const VecI<kSpatialNDim> inIdx1, const float q, const VecI<kSpatialNDim> outIdx)
{
    ...
    if constexpr (NumChannelsT::kHasStaticChannels)
    {
        using FloatT     = musa::ConvertBaseTypeTo<float, InT>;
        const FloatT a   = musa::StaticCast<float>(LoadPixelLdg(inWrap, numChannels, inIdx0));
        const FloatT b   = musa::StaticCast<float>(LoadPixelLdg(inWrap, numChannels, inIdx1));
        FloatT       tmp = b - a;
#pragma unroll
        for (int c = 0; c < NumChannelsT::kStaticChannels; c++)
        {
            musa::GetElement(tmp, c) = fmaf(musa::GetElement(tmp, c), q, musa::GetElement(a, c));
        }
        OutT &out = *GetWrapPtr(outWrap, outIdx);
        out       = musa::SaturateCast<OutT>(tmp);
    }
    else if constexpr (!NumChannelsT::kHasStaticChannels)
    {
        for (int c = 0; c < numChannels(); c++)
                                             ...
                                             const NumChannelsT numChannels)
{
    ...
    ForAllPixelsHorz(lo, hi,
                     [=](const VecI<kSpatialNDim> outIdx)
                     {
                         ...
                         const int   sx0i = musa::round<musa::RoundMode::DOWN, int>(sx0f);
                         const float q    = sx0f - sx0i;
                         const int   sx0  = musa::clamp(sx0i, 0, inShape.x - 1);
                         const int   sx1  = musa::clamp(sx0i + 1, 0, inShape.x - 1);
                                             ...
                                             const NumChannelsT numChannels)
{
    ...
    ForAllPixelsVert(lo, hi,
                     [=](const VecI<kSpatialNDim> outIdx)
                     {
                         ...
                         const int   sy0i = musa::round<musa::RoundMode::DOWN, int>(sy0f);
                         const float q    = sy0f - sy0i;
                         const int   sy0  = musa::clamp(sy0i, 0, inShape.y - 1);
                         const int   sy1  = musa::clamp(sy0i + 1, 0, inShape.y - 1);
                                              ...
                                              const NumChannelsT numChannels)
{
    ...
    for (int z = lo.z + threadIdx.y; z < hi.z; z += blockDim.y)
    {
        ...
        const int   sz0i = musa::round<musa::RoundMode::DOWN, int>(sz0f);
        const float q    = sz0f - sz0i;
        const int   sz0  = musa::clamp(sz0i, 0, inShape.z - 1);
        const int   sz1  = musa::clamp(sz0i + 1, 0, inShape.z - 1);
                                             ...
                                             const filter::ResamplingFilter filter, const NumChannelsT numChannels)
{
    ...
    for (int j = lo.x; j < hi.x; j += blockDim.x)
    {
        ...
        const int   sx0  = hugeSupport ? musa::round<musa::RoundMode::NEAREST, int>(sx0f)
                                       : musa::round<musa::RoundMode::UP, int>(sx0f);
        ...
        ForAllOrthogonalToHorz(
            ...
            [=](VecI<kSpatialNDim> outIdx)
            {
                ...
                if constexpr (NumChannelsT::kHasStaticChannels)
                {
                    ...
                    {
                        inIdx.x         = musa::clamp(sx0 + k, 0, inShape.x - 1);
                        const float flt = coeffs[coeffIdx];
                        const InT   px  = LoadPixelLdg(inWrap, numChannels, inIdx);
#pragma unroll
                        for (int c = 0; c < NumChannelsT::kStaticChannels; c++)
                        {
                            musa::GetElement(tmp, c) = fmaf(musa::GetElement(px, c), flt, musa::GetElement(tmp, c));
                        }
                    }

                    OutT &out = *GetWrapPtr(outWrap, outIdx);
                    out       = musa::SaturateCast<OutT>(tmp * norm);
                }
                else if constexpr (!NumChannelsT::kHasStaticChannels)
                {
                    for (int c = 0; c < numChannels(); c++)
                                             ...
                                             const filter::ResamplingFilter filter, const NumChannelsT numChannels)
{
    ...
    for (int i = lo.y; i < hi.y; i += blockDim.y)
    {
        ...
        const int   sy0  = hugeSupport ? musa::round<musa::RoundMode::NEAREST, int>(sy0f)
                                       : musa::round<musa::RoundMode::UP, int>(sy0f);
        ...
        ForAllOrthogonalToVert(lo, hi,
                               [=](VecI<kSpatialNDim> outIdx)
                               {
                                   ...
                                   if constexpr (NumChannelsT::kHasStaticChannels)
                                   {
                                       ...
                                       for (int k = 0; k < support; k++)
                                       {
                                           inIdx.y         = musa::clamp(sy0 + k, 0, inShape.y - 1);
                                           const float flt = coeffs[coeffBase + k];
                                           const InT   px  = LoadPixelLdg(inWrap, numChannels, inIdx);
#pragma unroll
                                           for (int c = 0; c < NumChannelsT::kStaticChannels; c++)
                                           {
                                               musa::GetElement(tmp, c)
                                                   = fmaf(musa::GetElement(px, c), flt, musa::GetElement(tmp, c));
                                           }
                                       }
                                              ...
                                              const filter::ResamplingFilter filter, const NumChannelsT numChannels)
{
    ...
    for (int i = lo.z; i < hi.z; i += blockDim.y)
    {
        ...
        const float sz0f = z * scale + srcZ0;
        const int   sz0  = hugeSupport ? musa::round<musa::RoundMode::NEAREST, int>(sz0f)
                                       : musa::round<musa::RoundMode::UP, int>(sz0f);
        ...
        for (int y = threadIdx.z + lo.y; y < hi.y; y += blockDim.z)
        {
            for (int x = threadIdx.x + lo.x; x < hi.x; x += blockDim.x)
            {
                ...
                if constexpr (NumChannelsT::kHasStaticChannels)
                {
                    ...
                    for (int k = 0; k < support; k++)
                    {
                        inIdx.z         = musa::clamp(sz0 + k, 0, inShape.z - 1);
                        const float flt = coeffs[coeffBase + k];
                        const InT   px  = LoadPixelLdg(inWrap, numChannels, inIdx);
#pragma unroll
                        for (int c = 0; c < NumChannelsT::kStaticChannels; c++)
...
{
    auto originV                    = musa::SetAll<VecF<kSpatialNDim>>(0.f);
    auto scaleV                     = musa::SetAll<VecF<kSpatialNDim>>(1.f);
    musa::GetElement(originV, axis) = origin;
    musa::GetElement(scaleV, axis)  = scale;
                                                  ...
                                                  const VecI<kSpatialNDim> hi, const NumChannelsT numChannels)
{
    ...
    int         axis   = musa::GetElement(sampleDesc.processingOrder, kWhichPass); // vec-order: 0 = X, 1 = Y, 2 = Z
    const float origin = musa::GetElement(sampleDesc.origin, kWhichPass);
    const float scale  = musa::GetElement(sampleDesc.scale, kWhichPass);
...

{
    ...
    hi = musa::min(hi, sampleDesc.shapes[kWhichPass + 1]);
...
inline auto srcDst(const nvcv::Tensor &src, const nvcv::Tensor &dst)
{
    ...
    if (!srcData)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "Input must be musa-accessible tensor");
    }

    if (!dstData)
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "Output must be musa-accessible tensor");
    }
...
inline VecI<kSpatialNDim> TensorShape(const HQResizeTensorShapeI &shape)
{
    ...
    {
        musa::GetElement(shapeVec, d) = shape.extent[kSpatialNDim - d - 1];
    }
    return shapeVec;
}
...
inline VecI<kSpatialNDim> TensorShape(const nvcv::Tensor &tensor)
{
    ...
    for (int d = 0; d < kSpatialNDim; d++)
    {
        ...
        musa::GetElement(tensorShape, d) = shape[axis];
    }
    return tensorShape;
}
...
public:
    ...
    VecI<ndim> operator()()
    {
        for (int i = 0; i < ndim; i++) musa::GetElement(m_bestOrder, i) = i;
        m_axisVisited = {};
        m_currSize    = m_inSize;
        m_minCost     = 1e+30f;
        Run(0);
...
private:
    ...
    void Run(int pass, float totalCost = 0)
    {
        ...
        else
        {
            ...
            {
                if (musa::GetElement(m_axisVisited, a))
                    continue;
                musa::GetElement(m_axisVisited, a)  = true;
                musa::GetElement(m_currOrder, pass) = a;
                auto prevSize                       = musa::GetElement(m_currSize, a);
                musa::GetElement(m_currSize, a)     = musa::GetElement(m_outSize, a);

                float passCost = PassCost(pass, a);
                Run(pass + 1, totalCost + passCost);

                musa::GetElement(m_currSize, a)    = prevSize;
                musa::GetElement(m_axisVisited, a) = false;
            }
        }
    }
    ...
    float PassCost(int pass, int axis)
    {
        ...
        float baseComputeCost = musa::GetElement(m_filterSupport, axis) * vol;
...
public:
    ...
    void operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::Tensor &src, const nvcv::Tensor &dst,
                    ...
                    const bool antialias, const HQResizeRoiF *roi) const
    {
        ...
        if (ws.musaMem.ready != nullptr)
        {
            NVCV_CHECK_THROW(musaStreamWaitEvent(stream, ws.musaMem.ready));
        }
        ...
        bool wideStride   = std::max(inMaxStride, outMaxStride) > musa::TypeTraits<int32_t>::max;
        ...
        RunTypedSwitch<IntermediateBaseT>(
            ...
            [&](auto dummySrcVal, auto intermediateVal, auto dummyDstVal, auto numChannelsVal)
            {
                ...
                static_assert(numStaticChannels == -1 || numStaticChannels == musa::NumElements<InT>);
                static_assert(musa::NumElements<IntermediateT> == musa::NumElements<InT>);
                static_assert(musa::NumElements<OutT> == musa::NumElements<IntermediateT>);
    ...
    void operator()(musaStream_t stream, const cvcuda::Workspace &ws, const BatchContainer &src,
                    ...
                    const NVCVInterpolationType magInterpolation, const bool antialias, const HQResizeRoisF rois) const
    {
        ...
        {
            NVCV_CHECK_THROW(musaEventSynchronize(ws.pinnedMem.ready));
        }
        if (ws.musaMem.ready != nullptr)
        {
            NVCV_CHECK_THROW(musaStreamWaitEvent(stream, ws.musaMem.ready));
        ...
        NVCV_CHECK_THROW(musaMemcpyAsync(sampleDescsGpu, sampleDescsCpu, numSamples * sizeof(SampleDescT),
...
private:
             ...
             musaStream_t stream) const
    {
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Input must be musa-accessible, varshape pitch-linear image batch");
        }

        if (!dstData)
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Output must be musa-accessible, varshape pitch-linear image batch");
        }
        ...
        RunTypedSwitch<IntermediateBaseT>(
            ...
            [&](auto dummySrcVal, auto intermediateVal, auto dummyDstVal, auto numChannelsVal)
            {
                ...
                {
                    static_assert(numStaticChannels == musa::NumElements<InT>);
                    static_assert(musa::NumElements<IntermediateT> == musa::NumElements<InT>);
                    static_assert(musa::NumElements<OutT> == musa::NumElements<IntermediateT>);
                    if (wideStride)
                    {
    ...
    std::enable_if_t<ndim == 2> RunPasses(const SampleDescT                              &sampleDesc,
                                          ...
                                          const cvcuda::Workspace &ws, musaStream_t stream) const
    {
        static_assert(kSpatialNDim == 2);
        constexpr bool kHasDynamicChannels = kNumStaticChannels == -1;
        // sample extent, spatial extents, optional dynamic channel extent
        constexpr int  kWrapNDim = 1 + kSpatialNDim + kHasDynamicChannels;
        using OutWrap            = musa::TensorNDWrap<OutT, kWrapNDim, StrideT>;
        using InWrap             = musa::TensorNDWrap<const InT, kWrapNDim, StrideT>;
        using InterWrap          = musa::TensorNDWrap<IntermediateT, kWrapNDim, StrideT>;
        ...
        if (ws.musaMem.ready != nullptr)
        {
            NVCV_CHECK_THROW(musaEventRecord(ws.musaMem.ready, stream));
        }
    }
                                          ...
                                          const cvcuda::Workspace &ws, musaStream_t stream) const
    {
        ...
        if (ws.pinnedMem.ready != nullptr)
        {
            NVCV_CHECK_THROW(musaEventRecord(ws.pinnedMem.ready, stream));
        }
                 ...
                 const PassInWrap &inWrap, int numSamples, musaStream_t stream) const
    {
        ...
        for (int sampleIdx = 0; sampleIdx < numSamples; sampleIdx++)
        {
            ...
            maxNumBlocks                   = musa::max(maxNumBlocks, numBlocks);
        }
    ...
    int RequiredSharedMemorySize(const SampleDescT &sampleDesc, int whichPass) const
    {
        ...
        int axis    = musa::GetElement(sampleDesc.processingOrder, whichPass);
                                         ...
                                         const HQResizeRoiF *roi) const
    {
        ...
        // according to the best processingOrder of axes
        {
            ...
            {
                const int   axis         = musa::GetElement(sampleDesc.processingOrder, pass);
                const int   axisOutShape = musa::GetElement(outShape, axis);
                const float roiStart     = musa::GetElement(parsedRoi.lo, axis);
                const float roiEnd       = musa::GetElement(parsedRoi.hi, axis);

                musa::GetElement(intermediateShape, axis) = axisOutShape;
                sampleDesc.filterKind[pass]               = filterKinds[axis];
                sampleDesc.filter[pass]                   = filters[axis];
                sampleDesc.shapes[pass + 1]               = intermediateShape;

                musa::GetElement(sampleDesc.origin, pass) = roiStart;
                musa::GetElement(sampleDesc.scale, pass)  = (roiEnd - roiStart) / axisOutShape;

                // "Clamp" the axes processed in later passes to the input ROI
                if (pass == 0)
                {
                    // the first processed axis roi is handled simply with the `origin`
                    musa::GetElement(sampleDesc.inRoiOffset, axis) = 0;
                }
                else
                {
                    ...
                    musa::GetElement(sampleDesc.shapes[0], axis)   = musa::GetElement(adjustedRoiSize, axis);
                    musa::GetElement(sampleDesc.inRoiOffset, axis) = musa::GetElement(adjustedRoi.lo, axis);
                    musa::GetElement(sampleDesc.origin, pass)
                        -= musa::GetElement(adjustedRoi.lo, axis); // parsedRoi.lo - adjustedRoi.lo
                }
            }
        }
    }
    ...
    shape::Roi<float, kSpatialNDim> ParseROI(const HQResizeRoiF *roi, VecI<kSpatialNDim> inShape) const
    {
        ...
        for (int dim = 0; dim < kSpatialNDim; dim++)
        {
            ...
            auto  axisSize = musa::GetElement(inShape, axis);
            ...
            musa::GetElement(retRoi.lo, axis) = roiStart;
            musa::GetElement(retRoi.hi, axis) = roiEnd;
        }
        return retRoi;
    }
                      ...
                      const filter::FilterMode &minFilter, const filter::FilterMode &magFilter) const
    {
        ...
        {
            float      inSize     = std::abs(musa::GetElement(roiShape, axis));
            float      outSize    = musa::GetElement(outShape, axis);
                                                     ...
                                                     const filter::ResamplingFilter         filters[kSpatialNDim]) const
    {
        ...
        for (int axis = 0; axis < kSpatialNDim; axis++)
        {
            const float &axisLo  = musa::GetElement(roi.lo, axis);
            const float &axisHi  = musa::GetElement(roi.hi, axis);
            ...
            const int axisSize = musa::GetElement(inShape, axis);
            musa::GetElement(ajustedRoi.lo, axis)
                = std::max<int>(0, std::min<int>(axisSize, std::floor(adjustedAxisLo)));
            musa::GetElement(ajustedRoi.hi, axis)
                                            ...
                                            const filter::ResamplingFilter filters[kSpatialNDim]) const
    {
        ...
        for (int i = 0; i < kSpatialNDim; i++)
        {
            ...
            musa::GetElement(filterSupport, i) = std::max(1, support);
        }
    ...
    std::enable_if_t<ndim == 2> SetupBlockLayout(SampleDescT &sampleDesc) const
    {
        ...
        for (int pass = 0; pass < kSpatialNDim; pass++)
        {
            int     resamplingAxis = musa::GetElement(sampleDesc.processingOrder, pass);
            ...
            musa::GetElement(blockShape, 1 - resamplingAxis) *= lanes;
            auto outputShape            = sampleDesc.shapes[pass + 1];
            sampleDesc.blockShape[pass] = musa::clamp(blockShape, VecI<2>{1, 1}, outputShape);
        }
    }
    ...
    std::enable_if_t<ndim == 3> SetupBlockLayout(SampleDescT &sampleDesc) const
    {
        ...
        for (int pass = 0; pass < kSpatialNDim; pass++)
        {
            ...
            if (resamplingAxis < 2)
            {
                ...
                sampleDesc.blockShape[pass] = musa::clamp(blockShape, VecI<3>{1, 1, 1}, outputShape);
            }
            else
            {
...
public:
    ...
    void operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::ImageBatchVarShape &src,
    ...
    void operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::TensorBatch &src,
...
// Tensor variant
void HQResize::operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::Tensor &src,
...
void HQResize::operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::ImageBatchVarShape &src,
                          const nvcv::ImageBatchVarShape &dst, const NVCVInterpolationType minInterpolation,
...
// TensorBatch variant
void HQResize::operator()(musaStream_t stream, const cvcuda::Workspace &ws, const nvcv::TensorBatch &src,
                          const nvcv::TensorBatch &dst, const NVCVInterpolationType minInterpolation,
                          const NVCVInterpolationType magInterpolation, bool antialias, const HQResizeRoisF rois) const
{
    assert(m_impl);
