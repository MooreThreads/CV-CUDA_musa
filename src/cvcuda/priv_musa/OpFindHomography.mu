...
#include <cvcuda/cuda_tools_musa/math/LinAlg.hpp>
...
typedef musa::math::Vector<float, 8>     vector8;
typedef musa::math::Vector<float, 9>     vector9;
typedef musa::math::Vector<int, 8>       intvector8;
typedef musa::math::Vector<float, 32>    vector32;
typedef musa::math::Matrix<float, 8, 8>  matrix8x8;
typedef musa::math::Matrix<float, 8, 32> matrix8x32;
typedef musa::math::Matrix<double, 8, 8> dmatrix8x8;
typedef musa::math::Vector<double, 8>    dvector8;
...
#define CUDA_CHECK_ERROR(err, msg)                                                                                  \
    do                                                                                                              \
    {                                                                                                               \
        musaError_t _err = (err);                                                                                   \
        if (_err != musaSuccess)                                                                                    \
        {                                                                                                           \
            std::cerr << "(" << musaGetErrorString(_err) << ") at line " << __LINE__ << " in " << __FILE__ << " : " \
                      << msg << std::endl;                                                                          \
            return;                                                                                                 \
        }                                                                                                           \
    }                                                                                                               \
...
#define CUBLAS_CHECK_ERROR(err, msg)                                                                                \
    do                                                                                                              \
    {                                                                                                               \
        mublasStatus_t _err = (err);                                                                                \
        if (_err != MUBLAS_STATUS_SUCCESS)                                                                          \
        {                                                                                                           \
...
#define CUSOLVER_CHECK_ERROR(err, msg)                                                                                \
    do                                                                                                                \
    {                                                                                                                 \
        musolverStatus_t _err = (err);                                                                                \
        if (_err != MUSOLVER_STATUS_SUCCESS)                                                                          \
        {                                                                                                             \
...
__device__ void printMatrixDevice(musa::math::Matrix<T, N> &A)
{
    for (int i = 0; i < N; i++)
    {
        printf("[");
...
template<typename T, int N>
__device__ void printMatrixDeviceParallel(musa::math::Matrix<T, N> &A)
{
    __threadfence();
    if (threadIdx.x < N)
    {
...
template<typename T, int N>
__device__ void printVectorDevice(musa::math::Vector<T, N> &x, int batch)
{
    printf("Batch = %d\n", batch);
...
template<typename T, int N>
__device__ void printVectorDeviceParallel(musa::math::Vector<T, N> &x, int batch)
{
    if (threadIdx.x < N)
...
__device__ int compute_model_estimate(float2 cM, float2 cm, float2 sM, float2 sm, float *W, float *V, vector8 &x,
                                      musa::Tensor3DWrap<float> model, int batch, int numPoints);
...
__device__ inline int getNumPoints(musa::Tensor2DWrap<float2> src, int numPoints, int batch)
{
    return numPoints;
}
...
template<class Func>
__device__ void reducef(float *data, musa::math::Vector<float, 32> &warpSums, float *result, Func op, int numPoints,
                        int batch)
{
    int      tid    = threadIdx.x;
...
template<class Func>
__device__ void reducef2(float2 *data, musa::math::Vector<float2, 32> &warpSums, float2 *result, Func op, int numPoints,
                         int batch)
{
    int      tid    = threadIdx.x;
    int      idx    = threadIdx.x + blockIdx.x * blockDim.x;
...
template<class Func>
__device__ void reduceLtL(float2 *src, float2 *dst, musa::math::Vector<float, 32> &warpSums, float *result, Func op,
                                      ...
                                      musa::Tensor3DWrap<float> model, int batch, int numPoints)
{
    ...
    musa::math::Matrix<float, 3, 3> tH0;
    musa::math::Matrix<float, 3, 3> tHtemp1;
    musa::math::Matrix<float, 3, 3> tHtemp2;
        ...
        if (!(ret || numPoints == 4))
        {
            ...
            while (true)
            {
                ...
                if (Sd < S)
                {
                    ...
                    if (tid < 8)
                        musa::math::detail::swap(x[tid], xd[tid]);
                    __syncwarp();
                    __syncthreads();
...
{
    int        batch = blockIdx.y;
    __shared__ musa::math::Vector<float2, 32> warpSums;
    if (batch < batchSize)
    {
        int     numPoints    = getNumPoints(src, maxNumPoints, batch);
                            ...
                            int batchSize)
{
    ...
    __shared__ musa::math::Vector<float, 32> warpSums;
    if (batch < batchSize)
    {
        int     numPoints = getNumPoints(src, maxNumPoints, batch);
...
{
    dim3                      block(256, 1, 1);
    musa::Tensor3DWrap<float> modelWrap = musa::CreateTensorWrapNHW<float>(models);
    ...
    musolverDnHandle_t cusolverH      = cusolverData->cusolverH;
    syevjInfo_t        syevj_params   = cusolverData->syevj_params;

    musaMemsetAsync(reinterpret_cast<void *>(srcMean), 0, batchSize * sizeof(float2), stream);
    musaMemsetAsync(reinterpret_cast<void *>(dstMean), 0, batchSize * sizeof(float2), stream);
    musaMemsetAsync(reinterpret_cast<void *>(srcShiftSum), 0, batchSize * sizeof(float2), stream);
    musaMemsetAsync(reinterpret_cast<void *>(dstShiftSum), 0, batchSize * sizeof(float2), stream);
    musaMemsetAsync(reinterpret_cast<void *>(J), 0, 2 * numPoints * 8 * batchSize * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(r), 0, 2 * numPoints * batchSize * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(LtL), 0, 81 * batchSize * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(W), 0, 9 * batchSize * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(calc_buffer), 0, numPoints * batchSize * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(cusolverBuffer), 0, lwork * sizeof(float), stream);
    musaMemsetAsync(reinterpret_cast<void *>(cusolverInfo), 0, batchSize * sizeof(int), stream);
    ...
    CUSOLVER_CHECK_ERROR(musolverDnSetStream(cusolverH, stream), "Failed to set musa stream in cusolver");
    CUSOLVER_CHECK_ERROR(musolverDnSsyevjBatched(cusolverH, MUSOLVER_EIG_MODE_VECTOR, MUBLAS_FILL_MODE_LOWER, 9, LtL, 9,
                                                 W, cusolverBuffer, lwork, cusolverInfo, syevj_params, batchSize),
                         "Failed to calculate eigen values using syevj");
#ifdef DEBUG
    CUDA_CHECK_ERROR(musaStreamSynchronize(stream), "synchronization failed after eigen value solver");
    std::vector<int> info(batchSize);
    musaMemcpyAsync((void *)info.data(), (void *)cusolverInfo, batchSize * sizeof(int), musaMemcpyDeviceToHost, stream);
    CUDA_CHECK_ERROR(musaStreamSynchronize(stream), "synchronization failed after copying back cusolverInfo");
    ...
    for (int b = 0; b < batchSize; b++)
    {
        ...
        CUDA_CHECK_ERROR(musaStreamSynchronize(stream), "failed to synchronize");
    }
#endif
...
inline void RunFindHomography(const nvcv::TensorDataStridedCuda &src, const nvcv::TensorDataStridedCuda &dst,
                              ...
                              const cuSolver *cusolverData, musaStream_t stream)
{
    ...
    using SrcDstWrapper = musa::Tensor2DWrap<float2>;
...
{
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.srcMean)), sizeof(float2) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.dstMean)), sizeof(float2) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.srcShiftSum)), sizeof(float2) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.dstShiftSum)), sizeof(float2) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.LtL)), 81 * sizeof(float) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.W)), 9 * sizeof(float) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.r)), 2 * maxNumPoints * sizeof(float) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.J)), 2 * maxNumPoints * 8 * sizeof(float) * batchSize);
    musaMalloc(reinterpret_cast<void **>(&(bufferOffset.calc_buffer)), maxNumPoints * sizeof(float) * batchSize);
    CUSOLVER_CHECK_ERROR(musolverDnCreate(&(cusolverData.cusolverH)), "Failed to create cusolver handle");
    CUSOLVER_CHECK_ERROR(musolverDnCreateSyevjInfo(&(cusolverData.syevj_params)), "Failed to create syevj params");
    CUSOLVER_CHECK_ERROR(musolverDnXsyevjSetTolerance(cusolverData.syevj_params, 1e-7),
                         "Failed to set tolerance for syevj");
    CUSOLVER_CHECK_ERROR(musolverDnXsyevjSetMaxSweeps(cusolverData.syevj_params, 15),
                         "Failed to set max sweeps for syevj");
    CUSOLVER_CHECK_ERROR(musolverDnXsyevjSetSortEig(cusolverData.syevj_params, 1),
                         "Failed to set sorting of eigen values in syevj");
    CUSOLVER_CHECK_ERROR(
        musolverDnSsyevjBatched_bufferSize(cusolverData.cusolverH, MUSOLVER_EIG_MODE_VECTOR, MUBLAS_FILL_MODE_LOWER, 9,
                                           NULL, 9, NULL, &(cusolverData.lwork), cusolverData.syevj_params, batchSize),
        "Failed to calculate buffer size for syevj");
    musaMalloc(reinterpret_cast<void **>(&(cusolverData.cusolverBuffer)), cusolverData.lwork * sizeof(float));
    musaMalloc(reinterpret_cast<void **>(&(cusolverData.cusolverInfo)), batchSize * sizeof(int));
}

FindHomography::~FindHomography()
{
    musaFree(bufferOffset.srcMean);
    musaFree(bufferOffset.dstMean);
    musaFree(bufferOffset.srcShiftSum);
    musaFree(bufferOffset.dstShiftSum);
    musaFree(bufferOffset.LtL);
    musaFree(bufferOffset.W);
    musaFree(bufferOffset.r);
    musaFree(bufferOffset.J);
    musaFree(bufferOffset.calc_buffer);
    musolverDnDestroySyevjInfo(cusolverData.syevj_params);
    musolverDnDestroy(cusolverData.cusolverH);
    musaFree(cusolverData.cusolverBuffer);
    musaFree(cusolverData.cusolverInfo);
}

// Operator --------------------------------------------------------------------

// Tensor input variant
void FindHomography::operator()(musaStream_t stream, const nvcv::Tensor &srcPoints, const nvcv::Tensor &dstPoints,
...
void FindHomography::operator()(musaStream_t stream, const nvcv::TensorBatch &srcPoints,
                                const nvcv::TensorBatch &dstPoints, const nvcv::TensorBatch &models) const
{
    ...
    for (int b = 0; b < srcPoints.numTensors(); b++)
    {
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Input src points must be musa-accessible, pitch-linear tensor");
        }
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "Input dst points must be musa-accessible, pitch-linear tensor");
        }
        ...
        {
            throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                                  "model must be musa-accessible, pitch-linear tensor");
        }
