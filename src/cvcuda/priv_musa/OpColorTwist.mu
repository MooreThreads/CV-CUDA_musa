...
using Vec = musa::math::Vector<T, N>;
template<typename T, int N, int M>
using Mat = musa::math::Matrix<T, N, M>;
...
inline auto __device__ GetAffineTransform(const TwistWrap &twist)
{
    ...
    using BT        = musa::BaseType<ValueType>;
    static_assert(musa::NumElements<ValueType> == 4);
...
#pragma unroll
        for (int j = 0; j < 4; j++)
        {
            affineTransform[i][j] = musa::GetElement(row, j);
        }
    }
                                         ...
                                         const Mat<TwistT, N, N + 1> transform)
{
    ...
    using T                          = musa::BaseType<DstT>;
    static constexpr int numChannels = musa::NumElements<SrcT>;
    static_assert(std::is_same_v<T, std::remove_const_t<musa::BaseType<SrcT>>>);
...
#pragma unroll
    for (int i = 0; i < N; i++)
    {
        in_vec[i] = musa::GetElement(src_pixel, i);
    }
    in_vec[N]              = 1;
...
#pragma unroll
    for (int i = 0; i < N; i++)
    {
        musa::GetElement(out_pixel, i) = musa::SaturateCast<T>(out_vec[i]);
    }
    // rewrite the extra channels unaffected
#pragma unroll
    for (int i = N; i < numChannels; i++)
    {
        musa::GetElement(out_pixel, i) = musa::GetElement(src_pixel, i);
    }
    dst[coord] = out_pixel;
}
...
inline __device__ void DoColorTwist(SrcWrapper src, DstWrapper dst, const int2 size,
                                    const musa::Tensor1DWrap<const ValueType> param)
{
    static_assert(musa::NumElements<ValueType> == 4);
...
template<class SrcWrapper, class DstWrapper, typename ValueType>
inline __device__ void DoColorTwist(SrcWrapper src, DstWrapper dst, const int2 size,
                                    const musa::Tensor2DWrap<const ValueType> param)
{
    static_assert(musa::NumElements<ValueType> == 4);
    auto transform = GetAffineTransform(param);
    DoAffineTransform(src, dst, size, transform);
...
inline void RunColorTwist(musaStream_t stream, const SrcData &srcData, const DstData &dstData,
                          const ColorTwistParam &param)
{
    ...
    if constexpr (std::is_same_v<SrcData, nvcv::TensorDataStridedCuda>)
    {
        ...
        int2 size = musa::StaticCast<int>(long2{inAccess->numCols(), inAccess->numRows()});
        ...
        {
            auto src = musa::CreateTensorWrapNHW<const T, int32_t>(srcData);
    ...
    else
    {
        ...
        musa::ImageBatchVarShapeWrap<const T> src(srcData);
                          ...
                          const nvcv::TensorDataStridedCuda &twistData, bool hasPerSampleTwist)
{
    ...
    {
        auto twist = musa::Tensor1DWrap<const TwistT>(twistData);
        RunColorTwist<SrcDestT>(stream, srcData, dstData, twist);
    }
    else
    {
        auto twist = musa::Tensor2DWrap<const TwistT>(twistData);
        RunColorTwist<SrcDestT>(stream, srcData, dstData, twist);
    }
}
                                ...
                                const nvcv::Optional<nvcv::TensorDataStridedCuda> &twistData)
{
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "The twist argument must be musa-accessible, pitch-linear tensor");
    }
...
void ColorTwist::operator()(musaStream_t stream, const nvcv::Tensor &src, const nvcv::Tensor &dst,
...
void ColorTwist::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &src,
