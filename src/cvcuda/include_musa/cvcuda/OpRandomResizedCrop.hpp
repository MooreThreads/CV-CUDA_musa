...
inline void RandomResizedCrop::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
                                          const NVCVInterpolationType interpolation)
{
    nvcv::detail::CheckThrow(cvcudaRandomResizedCropSubmit(m_handle, stream, in.handle(), out.handle(), interpolation));
}

inline void RandomResizedCrop::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in,
