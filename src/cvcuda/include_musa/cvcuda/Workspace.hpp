...
inline NVCVWorkspaceRequirements MaxWorkspaceReq(const WorkspaceRequirements &a, const WorkspaceRequirements &b)
{
    ...
    ret.musaMem   = MaxWorkspaceReq(a.musaMem, b.musaMem);
    return ret;
}
...
inline void AlignUp(WorkspaceRequirements &ws)
{
    ...
    ws.musaMem.size   = nvcv::detail::AlignUp(ws.musaMem.size, ws.musaMem.alignment);
}
...
public:
    ...
    UniqueWorkspace(WorkspaceMem host, WorkspaceMem pinned, WorkspaceMem musa, Deleter del = {})
        : m_impl{host, pinned, musa}
        , m_del(std::move(del))
    {
    }
...
inline UniqueWorkspace AllocateWorkspace(WorkspaceRequirements req, nvcv::Allocator alloc = {})
{
    ...
    auto del = [alloc](NVCVWorkspace &ws)
    {
        ...
        {
            if (ws.hostMem.ready)
                if (musaEventSynchronize(ws.hostMem.ready) != musaSuccess)
                    throw std::runtime_error("musaEventSynchronize failed");
        ...
        if (ws.pinnedMem.data)
        {
            if (ws.pinnedMem.ready)
                if (musaEventSynchronize(ws.pinnedMem.ready) != musaSuccess)
                    throw std::runtime_error("musaEventSynchronize failed");
        ...
        {
            if (ws.musaMem.ready)
                if (musaEventSynchronize(ws.musaMem.ready) != musaSuccess)
                    throw std::runtime_error("musaEventSynchronize failed");
            alloc.cudaMem().free(ws.musaMem.data, ws.musaMem.req.size, ws.musaMem.req.alignment);
            ws.musaMem.data = nullptr;
        }
    };
    NVCVWorkspace ws = {};
    try
    {
        ...
        if (req.musaMem.size)
            ws.musaMem.data = alloc.cudaMem().alloc(req.musaMem.size, req.musaMem.alignment);
