...
#include <cvcuda/cuda_tools_musa/MathWrappers.hpp> // for musa::max, etc.
#include <cvcuda/cuda_tools_musa/TypeTraits.hpp>   // for musa::Require, etc.
...
template<class T, int M, int N, class = musa::Require<(M == N && N > 1)>>
...
template<int R, typename T, int M, int N, class = musa::Require<R <= M>>
...
#pragma unroll
        for (int j = 0; j < N; ++j)
        {
            big = musa::max<F>(big, musa::abs(m[i][j]));
        }
...
template<class T, int N, class = musa::Require<(N < 4)>>
