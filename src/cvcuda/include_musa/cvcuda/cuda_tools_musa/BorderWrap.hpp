...
template<typename T>
constexpr inline bool __host__ __device__ IsOutsideActive(bool active, T c, T s)
{
    return active ? ((c < 0) || (c >= s)) : false;
}
...
constexpr inline T __host__ __device__ GetIndexWithBorderActive(bool active, T c, T s)
{
    static_assert(B != NVCV_BORDER_CONSTANT, "GetIndexWithBorderActive cannot be used with NVCV_BORDER_CONSTANT");
...
private:
    ...
    {
        return Base::m_tensorWrap.ptr(GetIndexWithBorderActive<kBorderType>(
            kActiveDimensions[Is], static_cast<StrideType>(c), Base::m_tensorShape[kMap.from[Is]])...);
    }
};
...
private:
    ...
    inline __host__ __device__ ValueType *doGetPtr(std::index_sequence<Is...>, Args... c) const
    {
        if ((IsOutsideActive(kActiveDimensions[Is], static_cast<StrideType>(c), Base::m_tensorShape[kMap.from[Is]]) || ...))
        {
            return nullptr;
        }
