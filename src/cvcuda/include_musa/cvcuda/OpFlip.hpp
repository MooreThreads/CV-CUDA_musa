...
public:
    ...
    void operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out, int32_t flipCode);
...
inline void Flip::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out, int32_t flipCode)
{
    nvcv::detail::CheckThrow(cvcudaFlipSubmit(m_handle, stream, in.handle(), out.handle(), flipCode));
}

inline void Flip::operator()(musaStream_t stream, const nvcv::ImageBatch &in, const nvcv::ImageBatch &out,
                             const nvcv::Tensor &flipCode)
{
