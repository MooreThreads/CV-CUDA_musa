...
constexpr inline IndexType __host__ __device__ GetIndexForInterpolation(float c)
{
    ...
    {
        return musa::round<RoundMode::DOWN, IndexType>(c);
    }
    ...
    else if constexpr (I == NVCV_INTERP_AREA)
    {
        if constexpr (Position == 1)
            return musa::round<RoundMode::UP, IndexType>(c);
...
protected:
    ...
    {
        musa::ConvertBaseTypeTo<int, DimType> ic;
...
private:
    ...
    {
        return musa::round<RoundMode::UP, int>(scaleX) == scaleX && musa::round<RoundMode::UP, int>(scaleY) == scaleY;
    }
