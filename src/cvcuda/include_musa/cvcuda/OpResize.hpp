...
inline void Resize::operator()(musaStream_t stream, const nvcv::Tensor &in, const nvcv::Tensor &out,
                               const NVCVInterpolationType interpolation)
{
    nvcv::detail::CheckThrow(cvcudaResizeSubmit(m_handle, stream, in.handle(), out.handle(), interpolation));
}

inline void Resize::operator()(musaStream_t stream, const nvcv::ImageBatchVarShape &in,
