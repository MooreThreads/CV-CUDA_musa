...
public:
    template<typename Predicate>
    std::optional<Payload> getIf(size_t minSize, Predicate &&pred, std::optional<musaStream_t> stream);

    auto get(size_t minSize, size_t minAlignment, std::optional<musaStream_t> stream)
    {
        return getIf(
            minSize,
            [=](const Payload &p)
    ...
    void put(Payload &&payload, std::optional<musaStream_t> stream);
...
private:
    template<typename Predicate>
    std::optional<Payload> tryGetPerStream(size_t minSize, Predicate &&pred, musaStream_t stream);
