...
void StreamOrderedCache<Payload, Item>::waitAndPurge()
{
    ...
    while (m_tail)
    {
        ...
        {
            auto err = musaEventSynchronize(m_tail->readyEvent());
...
template<typename Payload, typename Item>
void PerStreamCache<Payload, Item>::put(Payload &&payload, std::optional<musaStream_t> stream)
{
    musaEvent_t readyEvent = StreamCachePayloadReady(payload);
    bool        per_stream = readyEvent != nullptr && musaEventQuery(readyEvent) == musaErrorNotReady;
