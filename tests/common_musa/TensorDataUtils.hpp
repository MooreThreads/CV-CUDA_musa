...
template<typename T, class VecType, typename ST, typename CT,
         class       = nvcv::musa::Require<nvcv::musa::detail::IsSameCompound<ST, CT>>,
...
{
    return *reinterpret_cast<RT *>(&vec[nvcv::musa::dot(coord, strides)]);
}
...
void SetTensorTo(const TensorData &tensorData, DT data, int sample)
{
    ...
    for (int i = sample; i < totalSamples; ++i)
    {
        ...
        if (auto err = musaMemcpy(outSamplePtr, srcVec.data(), size, musaMemcpyHostToDevice))
        {
            char msg[1024] = {};
...
static void SetTensorToRandomValueFloat(const TensorData &tensorData, DT minVal, DT maxVal, int sample)
{
    ...
    for (int i = sample; i < totalSamples; ++i)
    {
        ...
        if (musaSuccess
            != musaMemcpy(tDataAc->sampleData(i), srcVec.data(), tDataAc->sampleStride(), musaMemcpyHostToDevice))
        {
...
void SetTensorFromVector(const TensorData &tensorData, std::vector<DT> &data, int sample)
{
    ...
    if (sample < 0)
    {
        ...
        {
            if (musaSuccess
                != musaMemcpy(tDataAc->sampleData(i), data.data(), tDataAc->sampleStride(), musaMemcpyHostToDevice))
            {
                throw std::runtime_error("CudaMemcpy failed");
            }
        }
    ...
    {
        if (musaSuccess
            != musaMemcpy(tDataAc->sampleData(sample), data.data(), tDataAc->sampleStride(), musaMemcpyHostToDevice))
        {
            throw std::runtime_error("CudaMemcpy failed");
        }
    }
...
void GetVectorFromTensor(const TensorData &tensorData, int sample, std::vector<DT> &outData)
{
    ...
    if (musaSuccess
        != musaMemcpy(outData.data(), tDataAc->sampleData(sample), tDataAc->sampleStride(), musaMemcpyDeviceToHost))
    {
        throw std::runtime_error("CudaMemcpy failed");
    }
...
static void SetImageTensorFromVectorPlanar(const TensorData &tensorData, std::vector<DT> &data, int sample)
{
    ...
    auto copyToGpu = [&](int j)
    {
        ...
        {
            if (musaSuccess
                != musaMemcpy2D(basePtr, tDataAc->rowStride(),
    ...
    auto copyToGpu = [&](int i)
    {
        ...
        if (musaSuccess
            != musaMemcpy2D(
                ...
                tDataAc->numCols() * tDataAc->numChannels() * sizeof(DT), tDataAc->numRows(), musaMemcpyHostToDevice))
        {
...
static void GetImageVectorFromTensorPlanar(const TensorData &tensorData, int sample, std::vector<DT> &outData)
{
    ...
    {
        if (musaSuccess
            != musaMemcpy2D(outData.data() + (i * (tDataAc->numCols() * tDataAc->numRows())),
                            tDataAc->numCols() * sizeof(DT), basePtr, tDataAc->rowStride(),
                            tDataAc->numCols() * sizeof(DT), tDataAc->numRows(), musaMemcpyDeviceToHost))
        {
...
static void GetImageVectorFromTensor(const TensorData &tensorData, int sample, std::vector<DT> &outData)
{
    ...
    if (musaSuccess
        != musaMemcpy2D(outData.data(), tDataAc->numCols() * sizeof(DT) * tDataAc->numChannels(),
...
{
    using BT  = nvcv::musa::BaseType<ST>;
    using BT4 = nvcv::musa::MakeType<BT, 4>;
    using CVT = std::conditional_t<sizeof(VT) == 1, int, VT>;

    std::cout << "I Printing buffer " << name << " with:\nI\tSize = " << vec.size() << " Bytes\nI\tShape = " << shape
              << "\nI\tStrides = " << strides << "\nI\tValues = " << std::endl;

    for (BT x = 0; x < (nvcv::musa::NumElements<ST> >= 1 ? nvcv::musa::GetElement(shape, 0) : 1); ++x)
    {
        if (endls & 0b1000)
            std::cout << "{" << std::endl;
        else
            std::cout << "{" << std::flush;
        for (BT y = 0; y < (nvcv::musa::NumElements<ST> >= 2 ? nvcv::musa::GetElement(shape, 1) : 1); ++y)
        {
            if (endls & 0b0100)
                std::cout << "  [" << std::endl;
            else
                std::cout << "  [" << std::flush;
            for (BT z = 0; z < (nvcv::musa::NumElements<ST> >= 3 ? nvcv::musa::GetElement(shape, 2) : 1); ++z)
            {
                std::cout << " " << std::flush;
                for (BT w = 0; w < (nvcv::musa::NumElements<ST> >= 4 ? nvcv::musa::GetElement(shape, 3) : 1); ++w)
                {
                    ST coord = nvcv::musa::DropCast<nvcv::musa::NumElements<ST>>(BT4{x, y, z, w});
...
{
    static_assert(nvcv::musa::NumElements<ST> >= 2);

    int widthIdx  = nvcv::musa::NumElements<ST> - 1;
    int heightIdx = nvcv::musa::NumElements<ST> - 2;
    int width     = nvcv::musa::GetElement(shape, widthIdx);
    int height    = nvcv::musa::GetElement(shape, heightIdx);

    int c0size = 1, c1size = 1;
    if constexpr (nvcv::musa::NumElements<ST> == 4)
    {
        c0size = nvcv::musa::GetElement(shape, 0);
        c1size = nvcv::musa::GetElement(shape, 1);
    }
    else if constexpr (nvcv::musa::NumElements<ST> == 3)
    {
        c1size = nvcv::musa::GetElement(shape, 0);
    }
