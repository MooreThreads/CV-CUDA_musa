    ...
    : m_planeStride(0)
{
    ...
    if (musaSuccess
        != musaMemcpy(m_data.data(), tDataAc->sampleData(sampleIndex), sampleStride, musaMemcpyDeviceToHost))
    {
        throw Exception(Status::ERROR_INTERNAL, "musaMemcpy filed");
    }
                                               ...
                                               std::vector<nvcv::Byte> &outData)
{
    ...
    {
        if (musaSuccess
            != musaMemcpy2D(outData.data() + (i * (tDataAc->numCols() * tDataAc->numRows()) * bytesPerC), dstWidth,
                            basePtr, tDataAc->rowStride(), dstWidth, tDataAc->numRows(), musaMemcpyDeviceToHost))
        {
...
void GetImageByteVectorFromTensor(const TensorData &tensorData, int sample, std::vector<nvcv::Byte> &outData)
{
    ...
    if (musaSuccess
        != musaMemcpy2D(outData.data(), tDataAc->numCols() * bytesPerPixel, tDataAc->sampleData(sample),
...
static void SetImageTensorFromByteVectorPlanar(const TensorData &tensorData, std::vector<nvcv::Byte> &data, int sample)
{
    ...
    auto copyToGpu = [&](int j)
    {
        ...
        for (int i = 0; i < tDataAc->numChannels(); ++i)
        {
            ...
            if (musaSuccess
                != musaMemcpy2D(basePtr, tDataAc->rowStride(), srcPtr, srcPitch, srcWidthBytes, tDataAc->numRows(),
